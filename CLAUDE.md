...
- **Edge defense in depth — all traffic must transit Cloudflare.** Three layers:
  ...
  2. **Origin secret** (`_enforce_origin_secret` in `security.py`): when `CF_ORIGIN_SECRET` is set, every request must carry the matching `X-Origin-Auth` header, injected at the edge by a Cloudflare Transform Rule — defeats direct-to-App-Engine access with a spoofed Host. Unset = disabled (local dev). **⚠️ STATE AS OF 2026-08-11: NOT IN SERVICE, on either side** — `cf-origin-secret` does not exist in Secret Manager (audit log: never created) and the zone has no `http_request_late_transform` ruleset. This layer has never run. It is what makes `CF-Connecting-IP` trustworthy, so while it is off, an attacker fronting `athena-pallas.nn.r.appspot.com` with *their own* Cloudflare zone clears layer 1 (their traffic comes from Cloudflare IPs), clears layer 3 (they choose the Host), and can forge that header — neutralising all three brute-force brakes (login 5/min, DAV 10/15 min, MCP bearer 20/15 min). Turning it on is edge-first, secret-second: the reverse 403s the whole site (see the Known Gotcha).
...
- **Cloudflare Access** (Zero Trust) is *intended* to front `/dav/*` with a service-token policy for DavX5 and a Google SSO policy for interactive use. **⚠️ STATE AS OF 2026-08-11: NOT CONFIGURED.** The account holds exactly one Access application (« Warp Login App », unrelated), and an anonymous `PROPFIND /dav/` answers `401 WWW-Authenticate: Basic realm="Pallas Athena"` — that is `dav_auth.py` replying, not Access. No commit in the repo's history ever provisioned it. DAV is therefore defended by the App Engine firewall + Basic bcrypt + the per-IP brake, and **that brake is only as good as layer 2 above**, which is also off. Enabling Access is edge-only (**no code reads `CF-Access-*`** — zero occurrences outside prose), so it is reversible without a deploy; the blocker is whether DavX5 can send `CF-Access-Client-Id`/`-Secret` as custom headers, which must be proven on the device BEFORE the application is created.
...
- **Edge security (state verified 2026-08-11, not aspiration):** App Engine firewall = the 22 published Cloudflare ranges + `0.1.0.2/32`, default `DENY` ✓ · Configuration Rule « MCP API » disabling Browser Integrity Check on `/mcp` + `/oauth/*` ✓ · portail rate limit ✓ · Rocket Loader `off`, Early Hints `on`, SSL `strict`, min TLS 1.3 ✓ · **Cloudflare Access on `/dav/*`: ABSENT** · **origin secret: ABSENT** · **edge HSTS 180 j, overriding the origin's 2 years** · apex `poirierlavoie.ca` has no address record, so HSTS preload reads `rejected`
- **Secrets:** Google Cloud Secret Manager — `flask-secret-key`, `firebase-api-key`, `dav-password-hash`, `portail-secret-key`, `graph-client-secret` (resolved by `config.py` at startup in production). **`cf-origin-secret` is listed by `config.py` but does NOT exist** — hence the disabled layer above. All five existing payloads verified free of stray whitespace (the trailing-newline trap below).
