...
| `/dossiers/<id>/tab/<tab_name>` | GET | HTMX tab loader. Leaf slugs: `apercu` (empty), `analyse` (théorie de la cause), `temps` — default, `facturation`, `fideicommis`, `audiences` (« Calendrier »), `taches`, `protocole`, `documents` (« Fichiers »), `notes`; legacy `agenda` maps to `audiences` |
