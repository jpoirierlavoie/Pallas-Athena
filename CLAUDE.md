...
- **Edge security (state verified 2026-08-11, not aspiration):** App Engine firewall = the 22 published Cloudflare ranges + `0.1.0.2/32`, default `DENY` ✓ · Configuration Rule « MCP API » disabling Browser Integrity Check on `/mcp` + `/oauth/*` ✓ · portail rate limit ✓ · Rocket Loader `off`, Early Hints `on`, SSL `strict`, min TLS 1.3 ✓ · **Cloudflare Access on `/dav/*`: ABSENT** · **origin secret: ARMED 2026-08-11** · **edge HSTS aligned to the origin's 2 years, 2026-08-11** · apex `poirierlavoie.ca` has no address record, so HSTS preload reads `rejected`
