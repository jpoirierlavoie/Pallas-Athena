...
- **Le texte source de la charte est GELÉ par un sha256 épinglé.** `BASE_CHARTER` et `SCHEDULED_ADDENDUM` SONT la version 1 : les éditer ferait mentir le registre sur chaque tour historique. Une correction se fait à l'écran, ce qui frappe une version Firestore (≥ 2). Même raison pour les **deux chemins de jointure** de `charter_text` : la version source se concatène BRUTE (normaliser changerait ses octets), une version Firestore a un joint explicite (un corps enregistré sans saut final collerait sinon l'addendum à son dernier paragraphe). Au lot de scission, `SOCLE ∪ SEED_CORPS` portait exactement les lignes de `BASE_CHARTER` — un test l'exigeait, pour prouver que la coupe ne changeait aucune règle. **Cette égalité a été RETIRÉE le même jour** : le socle a commencé à évoluer pour son compte (vouvoiement, style de réponse), ce qui est exactement ce à quoi sert un texte sous revue de code et ce qu'une égalité figée aurait interdit. Ce qui reste épinglé est ce pour quoi le socle existe : non vide, ses deux sections déontologiques présentes **avec leur compte de règles** (une sous-chaîne survit à la suppression d'une puce sur deux), les identifiants d'outils cités, et un registre unique — vouvoiement partout, là où `BASE_CHARTER`, gelé, reste au tutoiement.
