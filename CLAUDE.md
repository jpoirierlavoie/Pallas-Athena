...
| `/dossiers/<id>/tab/<tab_name>` | GET | HTMX tab loader. Leaf slugs: `apercu` (empty), `temps` — default, `facturation`, `fideicommis`, `audiences` (« Calendrier »), `taches`, `protocole`, `documents` (« Fichiers »), `notes`, `analyse` (théorie de la cause); legacy `agenda` maps to `audiences` |
