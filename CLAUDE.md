...
- **Security headers on every response** (via `security.py` `_add_security_headers` `after_request` hook):
  - `Content-Security-Policy` — **enforced** since 2026-07-11 (see the `CSP` constant in `security.py`; flipped after verifying against 90 days of report-only `/csp-report` data — only `script-src` ever reported violations). `script-src` is `'self' 'unsafe-inline' 'unsafe-eval'` + `ajax.cloudflare.com` + the Google origins the App Check SDK loads reCAPTCHA Enterprise from (`gstatic.com`, `apis.google.com`, `google.com`) — **no script CDN origins** (assets are vendored). The two `'unsafe-*'` tokens are documented necessities (Rocket Loader's un-nonced inline loader + the app's inline scripts/`on*` handlers; Alpine's `new Function()`); `style-src` keeps `'unsafe-inline'` for reCAPTCHA's dynamic inline styles. **Rocket Loader was disabled at the edge (2026-07-11)**, so `ajax.cloudflare.com` and the `script-src 'unsafe-inline'` are now removable once the inline `<script>` blocks and `on*` handlers are externalized. Violations are posted to `/csp-report` (`report-uri`, still active under enforcement) and logged as `csp_violation` security events.
...
- **CSP is ENFORCED** (since 2026-07-11; flipped after verifying against 90 days of report-only `/csp-report` data — only `script-src` ever reported). `script-src` retains `'unsafe-inline'`/`'unsafe-eval'` and `style-src` retains `'unsafe-inline'` as documented necessities (see the `CSP` constant in `security.py`). `report-uri` stays active, so violations are still collected under enforcement. Rocket Loader was disabled at the edge, so `ajax.cloudflare.com` + `script-src 'unsafe-inline'` can be dropped once inline `<script>`/`on*` handlers are externalized.
