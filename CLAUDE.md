...
- **`chat_skills/{uuid4}`** + **`versions/{n:06d}`** — runtime skills (§5): head-at-each-turn binding, the exact `(skill_id, version)` pairs recorded on every turn. `active` toggles; nothing deletes.
