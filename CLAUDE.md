...
│   │   │                           # factures → record_payment, reçus direct-à-GCS, conciliations
│   │   │                           # banque ET carte, exports CSV + PDF légal)
...
All `@login_required`, French UI, standard CSRF, POST+redirect with inline error boxes + 400. The receipt API endpoints exchange small JSON control messages (bytes go browser→GCS). First accounting module with a **nav entry** (base.html, both duplicated lists, icon `payments`).
