...
Direct deps beyond the original core set: `google-cloud-logging`, the OpenTelemetry stack (`opentelemetry-api`/`sdk` 1.44.0 + GCP trace exporter 1.13.0 / propagator 1.12.0 + Flask/requests/Jinja2 instrumentation 0.65b0 + an explicit `opentelemetry-resourcedetector-gcp` 1.14.0 pin — **all paired, they move as ONE atomic edit**: each instrumentation package hard-pins its siblings at `==`, so a single-package bump is unresolvable and Dependabot cannot automate it), `Pillow` (transitive via reportlab, pinned explicitly for CVE hygiene), and `defusedxml`. **There is no longer a `setuptools` pin** — contrib dropped `pkg_resources` at 0.49b0, so setuptools left the graph entirely (2026-07-30), which is what closed CVE-2026-59890 on all three scanning surfaces at once instead of suppressing it on each.
