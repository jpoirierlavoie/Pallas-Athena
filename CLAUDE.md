...
│   │
│   ├── mcp/                        # MCP connector — 49 tools: 27 read + 22 writes
...
│   │   └── handlers.py             # 49 tool implementations (27 read + 22 writes) over models/* + utils/*
│   │
...
│       │                           # material-symbols-outlined-v371-<sha8>.woff2 (icon ligature
...
```python
{
    ...
    "analyse": {                          # Le CACHE de la dernière analyse;
        ...
        "statut": "prete" | "en_attente" | "en_cours" | "echec"
                | "non_applicable",       # « prete » sur le chemin MCP ; les
...
- **Absolute-page mode — the leap controls (September 2026):** first / ±10 /
  last resolve a page NUMBER to `query.offset((p-1)*PAGE_SIZE)` on the
...
- `MANUAL_FIELDS` (deliberately data-less letter metadata: `objet_lettre`, `privilège`/`transmission_lettre` selects, `pièces_jointes` default `"Aucune"`, `référence_externe`, …). **`salutations` was removed** — it is passthrough. An `options` entry is a bare string (label == value) **or** a `(label, value)` couple; `privilège`'s « (aucune mention) » carries the sentinel `EMPTY_OPTION_VALUE` and prints NOTHING — it cannot be `""`, which an untouched select also submits and which means « [À COMPLÉTER : …] ». `_collect_values` now REFUSES a value outside a field's options (the `<select>` was the only guard until September 2026).
...
- **Un reCAPTCHA INVISIBLE ne nettoie pas son conteneur : il faut le REMPLACER, et `clear()` ne le fait pas.** Trois gardes du SDK vendorisé (`static/vendor/firebase-auth-compat-10.12.2.js`) se lisent ensemble et disent toutes la même chose : `clear(){…this.isInvisible||this.container.childNodes.forEach(…)}` ne retire AUCUN nœud, `validateStartingState(){…isInvisible||!this.container.hasChildNodes…}` SAUTE l'assertion « conteneur déjà sale », et `makeRenderPromise: …isInvisible||(t=document.createElement("div"),e.appendChild…)` rend **sur le conteneur lui-même** au lieu d'un enfant jetable. Un second `grecaptcha.render()` sur le même élément lève donc une `Error` **NUE — sans `.code`**, que `messageErreur` ne peut pas nommer et que le journal serveur, qui ne transporte que le code, ne voit pas passer. Le symptôme en production (2026-09-14) : « Ajouter un numéro » → « Impossible d'envoyer le code. Réessayez. », **zéro `StartMfaEnrollment` dans Identity Platform** — la requête n'a jamais quitté le navigateur, ce qui réfute d'un coup toute hypothèse côté serveur (fraude aux SMS, règles régionales, quota). Deux corollaires. **Le défaut est STRUCTUREL, pas intermittent** : « Ajouter un numéro » est `:disabled="!deverrouille"` et `deverrouille` ne se pose que par le déverrouillage, qui vient de rendre dans ce conteneur — l'inscription est donc TOUJOURS le second rendu, sans aucun chemin vers un premier, d'où une reproductibilité de 100 %. Et **la page de connexion portait le même défaut, latent** : `resetRecaptcha()` → `initRecaptcha()` est atteint depuis `sendCode` ET `resendCode`, donc le premier envoi marchait et « Renvoyer le code » aurait levé — sur la page de CONNEXION. La réparation est la même des deux côtés et elle est épinglée par DÉRIVATION dans `tests/test_mfa_recaptcha.py` (le remplacement existe, il PRÉCÈDE la construction, le nœud neuf reprend l'identifiant, et **tout gabarit construisant un `RecaptchaVerifier` doit figurer à la table** — les deux pages ont été écrites à onze mois d'intervalle avec le même défaut, une troisième le copierait). Passer l'ÉLÉMENT plutôt que son identifiant est sûr : `"string"==typeof t?document.getElementById(t):t`.
...
- **The pagination bar is ICON-ONLY since 2026-09-11, and that cost a font re-subset.** Six glyphs, no label text: `chevron_left`/`chevron_right` (walk), `keyboard_double_arrow_left`/`_right` (±`jump_size`), `first_page`/`last_page` (ends). FOUR of them were absent from the vendored subset, so the change was not a template edit — it was `MATERIAL_ICONS` 41 → 45, a woff2 regeneration (v369 → **v371**), and the full seven-file asset fan-out (`app.<hash>.css` rehash → `base.html` ×3 + `sw.js` PRECACHE + `STATIC_CACHE` v12 → v13 + `_EARLY_HINTS_*` ×2 + the literal in `test_security_headers.py`). Three consequences to keep: a glyph is the ONLY affordance, so every control carries `aria-label` **and** `title` (`ms()` renders `aria-hidden`, so the glyph contributes nothing to the accessible name, and with no text a hover/long-press is the only way to learn what a button does); the destination number that « Fin (42) » used to say out loud now lives in the last-page `title`, the counter's `/ 42` being the other half; and dropping the labels SHRANK the hit area, so the padding became the Rule-9 constraint — see the next entry. ⚠ **`.ms-20` is 21 px, not 20** (`static/src/app.input.css:129` — the `.ms-N` names are deliberately offset from their pixel size, « relevées de +1 px … décision 2026-08-10 »), which is why this line read « 54 × 46 » and « ~409 px » until 2026-09-11 and both were wrong. Any arithmetic on this bar starts by reading that rule, never the class name.
- **ONE row since 2026-09-11, and the COUNTER is what made it possible — not the button size.** `px-3 py-3` + a 21 px glyph + a 1 px border is **47 × 47** (the old `px-4` was 55 × 47), so six of them are **282 px** against ~343 px of usable width at 375 px (`max-w-4xl mx-auto px-4` on all seven list pages, and all 7 include sites sit at their partial's top level with no further padding). The « ~409 px, so two rows, arithmetic not taste » that preceded it counted the counter **INLINE**; moving it onto its own line below freed the ~85 px it ate, and the buttons only had to shrink one step — `px-2` would be 39 px and break Architecture Rule 9, so `px-3` is the only step available. **`justify-between` with NO gap, never `justify-center gap-2`:** padding is `rem` while the glyph is `px`, so at a browser root font-size of 18 px the gapped row needs 345 px against 339 px and **clips the last control** — « Fin », the one whose `title` carries the destination number. Edge-to-edge needs only 6× the button (300 px at root 18, 318 at 20), survives to ~root 21, and widens the separation between six adjacent 47 px targets from 8 px to 12.2 px; `flex-wrap` is the backstop past that, and `w-full` is load-bearing (a content-sized row leaves `justify-between` nothing to distribute). The six controls are ONE Jinja macro, not six copies, so « every control carries the same hx contract » became structural instead of hand-maintained. **The counter's `role="status"` does NOT announce, before or after the move**: the node lives INSIDE the htmx swap target (each `_X_rows.html` partial IS the container's whole content), so the live region is destroyed and re-inserted rather than mutated, and no screen reader announces a region that arrives already populated. Do not claim it does — an announcement needs an empty live region OUTSIDE the target, fed by `hx-swap-oob`.
- **A control is RENDERED per LIST and DISABLED per PAGE — and the two columns are NOT independent.** `_nav` emits `first_possible`/`last_possible`/`jump_possible` beside `show_first`/`show_end`/`jump_*_page`: the first three are the existential closure of the per-page rule over `page ∈ [1..L]` (both ends ⟺ `L ≥ 3`, both leaps ⟺ `L ≥ JUMP_PAGES + 2`), so the row keeps a constant shape for a whole walk of one list — the stability the practitioner asked for, « le bouton sous le doigt ne bouge jamais » — while no permanently dead control is ever drawn. Slot counts, constant per list: 2 pages → 2 · 3-11 → 4 · ≥12 → 6 · no total → 3. **`enabled ⟹ rendered` is a REQUIREMENT, not a nicety**, because `page` is deliberately not clamped to the total on the cursor path, so `page > last_page` is a reachable production state: the implication holds by arithmetic for `show_end`/`jump_next` (each bounds `L` from below on its own), by the `in_range = has_total and page <= last_page` term for `jump_prev`, and by an explicit `or show_first` in `first_possible`. That `in_range` term fixed a **shipped defect**: at page 40 of a 3-page list the bar offered a « Reculer de 10 pages » whose target `resolve_page` clamped to the stale total, so it went back **37** pages and reset the label to « Page 3 / 3 » — a control not doing what its own title says. The two end flags coincide at `L ≥ 3` by arithmetic ACCIDENT (each rule happens to cost two pages), not by derivation, so they stay two expressions; one shared flag would drift the day either rule moved. They also change nothing on any list measured in production and fire only at **L == 2**, which any filtered list of 16-30 rows reaches and where without them most of the row would be grey on every page. Two things this rule does NOT buy: `has_total` is recomputed per request, so an *intermittently* failing count still flips the bar 6 ↔ 3 mid-walk; and `disabled` removes a button from the tab order, so a keyboard user's tab-stop count still changes as they walk — the instability moved off the finger, not out of the page.
- **A disabled control carries NO `hx-*` attribute, and its class string OMITS `hover:`/`cursor-pointer`.** `:disabled` is not a guard (DevTools re-enables any button — the standing MFA gotcha), and omitting the wiring also keeps a `None` jump target out of the emitted JSON. The hover half is subtler: CSS `:hover` matches a disabled element and Tailwind adds no `:not(:disabled)`, so an inert button lights up — and that rule lives inside `@media (hover:hover)`, which makes the defect **invisible on the practitioner's phone and visible on a reviewer's desktop**. There is no `disabled:bg-*` in the compiled artifact, so omission is the only fix that avoids the seven-file fan-out; the artifact carries exactly four `disabled:` variants (`opacity-50`, `opacity-60`, `cursor-not-allowed`, `text-gray-400`), and `disabled:opacity-60` is preferred over `text-gray-400` — gray-400 on white is ≈2.5:1 and reads as a rendering artefact rather than an unavailable control, while gray-700 at 60 % is ≈3.4:1 and dims the border too. ⚠ **Never detect a disabled button with `"disabled" in html`**: `disabled:opacity-60` puts that substring on every LIVE button too, so the check goes **vacuously green** and silently stops verifying the hx contract it guards. `tests/test_pagination_component.py` anchors on `_DISABLED_RE = re.compile(r"\sdisabled(?=[\s>/])")` — and the identical trap caught `"cursor"` (inside `disabled:cursor-not-allowed`) one assertion later, so compare JSON **keys** (`'"cursor"'`), never bare words.
- **The pagination bar renders INSIDE each partial's `{% if rows %}`, so an overshooting jump would be a dead end.** All 7 include sites sit inside a rows-truthy guard, which is what makes an authoritative total over a failed read structurally impossible — but it also means a page that renders empty shows **no controls at all**. Hence `resolve_page` clamps to the known total, and where the total is unknown **no leap that needs one is rendered** — so no unclamped jump can be issued from the UI at all. The ONE exception is « Début », and it is exactly not an exception to the cost rule: page 1 is offset 0, so `resolve_page(1, …)` is `(1, 0)` under any total and the control cannot lie. Since 2026-09-11 it therefore renders on EVERY page of a countless list (disabled on pages 1-2), because it is the only way home there — « Précédent » moves one page via the trail, the trail caps at `MAX_TRAIL = 20`, and a failed count is the DEEP case by nature (a count fails on big collections, not small ones). Never loosen the outer guard to « show the bar anyway »: that resurrects an empty bar on every short legacy list.
...
- **A manual gabarit field must be read through `manual_spec()`, never `MANUAL_FIELDS[name]` — and the asymmetry that made it necessary is worth knowing.** Until 2026-09-05 `classify_placeholders` matched auto fields through `_canonical_for` (`.lower()`) and manual fields through a bare `elif name in MANUAL_FIELDS` — **exact equality**. `GABARITS_PLACEHOLDERS.md` announced « Matching is case-insensitive » without reserve, so the documentation described a rule the code applied to only half the fields. The consequence was measured in production, not hypothesised: both correspondence gabarits (« Lettre d'engagement », « Lettre de correspondance ») write `{{PRIVILÈGE}}` **in capitals** — which is how the mention is printed on a letter, and which the ALL-CAPS rule invites — so `PRIVILÈGE` landed in `passthrough` (a grey « À compléter dans Word » chip) while `{{transmission_lettre}}` in the same document got its `<select>`. The proof is in the stored inventories: « Demande introductive d'instance » writes `DISTRICT`/`TRIBUNAL`/`DEMANDEUR`/`DOSSIER.DEFENDEUR` in capitals with an **empty** `passthrough`, and the ONE capitalised name that ever fell through is the one that happens to be *manual*. The fix folds case on both branches (**case only — `.lower()` is not accent folding, so `{{privilege}}` unaccented is still an unknown name**, deliberately, because the auto branch behaves the same). ⚠ It also arms a **`KeyError` in three readers** that index `MANUAL_FIELDS[name]` on a name straight out of `classification.manual` — `routes/doc_templates`, **`routes/invoices`** and **`utils/note_docx`**; the last two never prompt, so a `{{PRIVILÈGE}}` in a note-d'honoraires or note-print template would 500 with nothing on screen to suggest why. All three go through `manual_spec`/`manual_value` now, and a test covers each. `manual_value` is also the ONE place the ALL-CAPS rule lives for manual fields (`note_docx` had grown a second implementation, `name.isupper()`). Option entries are `str` **or** `(label, value)`: « (aucune mention) » carries the sentinel `EMPTY_OPTION_VALUE` and prints nothing, which **cannot** be `""` — an untouched select submits `""` too, and that means `[À COMPLÉTER : …]`. `_collect_values` refuses an off-list value by name; before that the `<select>` was the only guard, so a stale tab could print any mention at all into a letter.
- **A gabarit value carrying a blank line must round-trip through a `<textarea>`, never an `<input type="text">`.** The generation popup pre-fills each resolved auto value into a form field and re-reads it from `request.form` on submit — so the value makes a full HTML round trip. The HTML *value sanitization algorithm* for `input[type=text]` **strips newlines**, which would silently flatten a multi-paragraph value into one line; `docx_fill`'s block expansion keys off `_BLANK_LINE_RE`, so the expansion would then simply never fire and every party would land in one run-on paragraph, with no error anywhere. `_fields_context` therefore flags a field `multiline` when its resolved value contains a blank line, the template renders a `<textarea>` (reusing `gabarits/form.html`'s class string verbatim — **no new Tailwind class, no recompile**), and `_collect_values` caps a multiline field at `_MULTILINE_MAX_CHARS` (20 000) instead of the letter-metadata `_SCALAR_MAX_CHARS` (2 000), which five parties with their addresses already exceed. This is why the multi-party intitulé rides block expansion rather than a paragraph-level `{{#region}}…{{/region}}`: the latter needs a **span regex**, the exact shape that cost this repo 43,3 s of ReDoS twice (`docx_fill._apply_conditions`), while `_apply_rows` only repeats a `<w:tr>` and **flattens newlines to spaces**, so an address could never occupy its own line in a cell.
...
- **La table cron compte TROIS entrées** (réconciliation portail, Bookings, miroir Outlook) — le piège REPLACE de `cron.yaml` vaut plus que jamais : `gcloud app deploy cron.yaml` remplace la table ENTIÈRE, donc un fichier partiel arrête les trois en silence. Une quatrième entrée (le répartiteur des tâches planifiées du clavardage) a été retirée le 2026-09-02, et son en-tête annonçait encore QUATRE : **le compte vit dans un commentaire que rien ne vérifie**, donc il se corrige à la main, dans le même commit. Un TROISIÈME service a de même vécu dans trois inventaires codés en dur qui ne se mettent pas à jour seuls — la boucle de prune de `cloudbuild.yaml`, le tuple yaml de `tests/test_security_headers.py`, `_SCAN_FILES` de `scripts/check_config.py` : le prochain service ajouté devra passer par les trois.
- **Le clavardage interne a existé du 2026-08-26 au 2026-09-02, et ce qui reste ici de lui est délibérément court.** Phase N était un client de conversation Claude/Gemini sur Vertex AI, sur un TROISIÈME service App Engine, avec son registre append-only, ses compétences et sa charte versionnées, ses tâches planifiées et un lecteur de la boîte Outlook du juriste. Il n'existait que pour que le matériel privilégié ne transite pas par un produit IA grand public ; le cabinet est passé à un compte Claude for Work couvert par une entente de traitement des données, et cette raison est tombée. Les données ont été effacées et vérifiées à zéro, le service et sa file supprimés. Une trentaine de Known Gotchas enseignaient ce sous-système ; ils sont partis avec lui, parce qu'un piège qui décrit du code absent coûte de l'attention sans jamais rien protéger. **Trois leçons survivent, parce qu'elles ne parlaient pas du clavardage :** (1) *un faux Firestore qui accepte ce que le vrai magasin refuse ne prouve rien* — les deux harnais de la suite acceptaient un tableau imbriqué, que Firestore rejette, et le défaut n'a paru qu'au premier appel réel, après l'appel de modèle déjà payé ; il y avait DEUX harnais à corriger, ce qui est exactement comment un seul aurait laissé passer le suivant. (2) *un test qui assertionne sur un dictionnaire remis à un mock n'est pas un test de ce qui est STOCKÉ* — trois tests couvraient le chemin dont le défaut vivait précisément à la frontière qu'ils simulaient, et 3 164 tests verts ne prouvaient rien ; là où `sanitize` est sur le trajet, il faut passer par le vrai helper du modèle. (3) *un outil de LECTURE nouvellement déployé est atteignable par le jeton EN VIGUEUR* — voir l'entrée voisine, qui reste vraie et vaut pour tout élargissement futur.
- **`Mail.Send` et `Calendars.ReadWrite` sont des grants Entra à l'échelle de l'ORGANISATION, que rien ne porte — et les porter exige de les RETIRER d'Entra d'abord.** Les deux alimentent trois sous-systèmes vivants (courriel d'invitation du portail, synchro Bookings, miroir Outlook) via `utils/graph.py` et le secret `graph-client-secret`. La documentation Microsoft de « RBAC for Applications » est explicite : une permission détenue à l'échelle de l'organisation dans Entra **ET** portée dans Exchange RBAC donne « **no effective resource scoping** » — les deux autorités s'ADDITIONNENT, l'application obtient la permission sur TOUTES les boîtes et la portée est décorative. C'est pourquoi le `Mail.ReadWrite` du lecteur de boîte (2026-08-28 → 2026-09-02) n'a jamais été accordé dans Entra mais UNIQUEMENT dans Exchange RBAC : retirer l'assignation de rôle et le scope a donc retiré toute la capacité, sans laisser un grant Entra redevenir non porté. Deux corollaires opérationnels : l'`ObjectId` attendu par `New-ServicePrincipal` est celui des **Applications d'entreprise**, pas celui des Inscriptions d'applications (qui en affiche un autre) ; et un changement de permission met **30 minutes à 2 heures** à sortir du cache Exchange, donc un accès qui fonctionne encore juste après un retrait n'est pas un échec du retrait — `Test-ServicePrincipalAuthorization` est la vérification. ⚠ Exchange PowerShell n'accepte **que** l'auth par certificat pour l'app-only ; `graph-client-secret` ne peut donc pas servir à administrer ce RBAC, et l'opération reste manuelle et interactive.
...
> **Timing (audit 2026-08-26, révisé 2026-09-02).** The suite itself is negligible against the build budget: it collects with zero errors and runs in **~12 s** locally. The budget is consumed by the pip install and the deploy steps — the LIVE `cloudbuild.yaml` carries FOUR `gcloud app deploy` steps (default, portail, dispatch, cron) under `timeout: '1800s'`; a fifth, the `chat` service, left on 2026-09-02. The yaml transcribed above is the pre-portail 3-step shape. **Watch a real build's wall time in Cloud Build history after each new service is added** — two service deploys leave comfortable margin in the 30 minutes, three did not.
...
### Le clavardage interne — Phase N, retirée (2026-08-26 → 2026-09-02)

Un client de conversation Claude/Gemini **sur Vertex AI** (« Assistant » dans la nav), livré pour que le matériel privilégié ne transite jamais par un produit IA grand public : inférence sous le projet GCP du cabinet, registre append-only, outillage attribué et versionné. Un tour s'exécutait en chaîne de Cloud Tasks sur un **TROISIÈME service App Engine** (`chat.yaml`, gunicorn `--timeout 570` — le `--timeout 60` du service default rendait l'exécution sur place impossible), et le navigateur POLLAIT (le premier `hx-trigger="every 2s"` du dépôt, arrêté par HTTP 286). Deux lots l'ont suivi : la **charte éditable** (2026-08-27 — socle sous revue de code, corps versionné en base) et la **messagerie Outlook** (2026-08-28 — lecture bornée de la boîte du juriste, pièces jointes, brouillons, jamais d'envoi).

**Retiré le 2026-09-02.** Le cabinet est passé à un compte **Claude for Work couvert par une entente de traitement des données**, ce qui retire au clavardage sa seule raison d'être ; il sous-performait par ailleurs nettement le modèle atteignable directement. Décisions du praticien : effacer les données sans exporter (la récupération Firestore de 7 jours et les sauvegardes de 90 jours suffisent), perdre la capacité de lecture de la boîte Outlook, abandonner les tâches planifiées, retirer entièrement les brouillons. 87 fichiers, 22 592 lignes, 396 tests ; 15 conversations, 173 enfants de sous-collection et 11 objets Storage (57,9 Mo) effacés puis vérifiés à zéro, service et file supprimés.
