...
│   │
│   ├── mcp/                        # MCP connector (Phase I + L) — 17 read + 2 note writes
...
│   │   └── handlers.py             # 19 tool implementations (17 read + 2 note writes) over models/* + utils/*
│   │
...
The Phase-I MCP layer added 14 tools; Phase K adds 3 read-only trust tools (17); **Phase L adds 2 note-write tools (19 total)**. The tool handlers in `mcp/handlers.py` compose **existing** model/util functions only; filters the model layer lacks are applied in the handler over a bounded fetch (≤ 200 docs), and **no composite index exists for an MCP-only query**. **Only the two handlers named in `mcp.tools.WRITE_TOOLS` write to Firestore, and only to the `notes` collection** — every other handler stays strictly read-only (notably: `list_protocol_steps` derives overdue status by date comparison instead of calling `check_overdue_steps`, which writes; the trust tools call `models/trust.py` read paths only). Note that the *request* path writes outside the tool path regardless (`bearer.stamp_token_last_used`, `oauth.touch_client`). **Trust tools never emit the bank transit or account number.**
...
- **`normalize_email`'s length bound is a ReDoS defense, not cosmetics.** `_EMAIL_RE` (`^[^@\s]+@[^@\s]+\.[^@\s]+$`) is **quadratic**: both classes after the `@` match the dot, so on `x@` + `a.`×n + `@` the engine tries every split point and rescans the tail — measured 3.8 s at 40 KB, ~96 min at the portal's 1 MB body cap, and `re` holds the GIL, so ONE request freezes the whole gunicorn worker (all 4 threads) until the 60 s timeout. The portal's **public, unauthenticated** `POST /api/renvoi` passes a JSON field straight in (App Check + 5/h per IP are the only other gates). The fix is the `len(normalized) > EMAIL_MAX_LENGTH` check (254, RFC 5321) **before** the match — never remove it, and never "fix" it by truncating instead (truncation would turn a 300-char address into a plausible-looking one). Note the shape CodeQL reports (trailing space) is **defused by `.strip()`** and matches in microseconds; a regression test built on it pins nothing — `tests/test_validators.py::test_email_redos_shape_returns_fast` uses a trailing `@` for exactly that reason. Same trap sits latent in `logging_setup.EMAIL_RE` (also quadratic, also unanchored), safe **only** because `_redact_string`'s `MAX_LEN = 2048` early return precedes it — keep that ordering.
