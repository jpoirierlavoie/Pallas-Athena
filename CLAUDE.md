...
│   │   │                           # (two-level tab bar) + _tab_apercu, _tab_temps,
...
| `/dossiers/<id>/tab/<tab_name>` | GET | HTMX tab loader. Leaf slugs: `apercu` (empty), `temps` — default, `facturation`, `fideicommis`, `audiences` (« Calendrier »), `taches`, `protocole`, `documents` (« Fichiers »), `notes`; legacy `agenda` maps to `audiences` |
