...
- **Exploitation, ordonné :** (1) déployer ; (2) `python -m scripts.purge_encaissements_factures` en simulation, LIRE la liste, puis `--apply` ; (3) ressaisir les cinq — les trois du fidéicommis **depuis le fidéicommis** (contre-passer le virement, le refaire en nommant le compte), les deux autres directement au registre ; (4) `python -m scripts.verify_admin_integrity` : aucun écart.
