...
- **`chat_draft`** — head + append-only `versions`; `create_draft` (head + v1 in one batch; a caller-supplied id never honoured), `revise_draft` (transactional n+1 + head move; unknown id refuses, NEVER creates), `validate_payload` (the handlers' dry-run seam), `get_draft`/`get_version`/`list_versions`, `list_drafts` (bounded global + Python filter — no composite). `PROVENANCE` ContextVar, whitelisted keys. **No delete function exists** (pinned).
