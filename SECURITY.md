...
- **DAV auth:** Separate HTTP Basic Auth (bcrypt-hashed password) plus a per-IP brute-force brake, behind the same App Engine firewall and origin-secret check as every other path. Cloudflare Access is deliberately **not** used (decision 2026-08-11: DAVx⁵ cannot send the custom headers a service token needs, and the client-certificate alternative would enable mTLS on a hostname shared with the web UI and the MCP connector).
- **Edge:** Cloudflare in front of App Engine with Full Strict TLS, and an App Engine firewall that admits only Cloudflare's published IP ranges. Direct App Engine access (`*.appspot.com`) is rejected by a `before_request` hook. The origin-secret header check (`X-Origin-Auth`) is **armed** since 2026-08-11: a zone-wide Cloudflare Transform Rule injects the header and `security.py` rejects any request without it.
- **Headers:** HSTS (2-year, origin and edge aligned 2026-08-11), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, restrictive `Permissions-Policy`. CSP is **enforced** (since 2026-07-11) with a per-request nonce on `script-src` (no `'unsafe-inline'`); violations are still collected via `report-uri` → `/csp-report`.
