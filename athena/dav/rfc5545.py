"""RFC-5545 CalDAV endpoints for tasks (VTODO) and dossier journals (VJOURNAL).

Endpoints — Tasks
------------------
/dav/tasks/                      Task collection (CalDAV with VTODO)
/dav/tasks/<task_id>.ics         Individual task resource

Endpoints — Journals
---------------------
/dav/journals/                   Journal collection (CalDAV with VJOURNAL)
/dav/journals/<dossier_id>.ics   Individual journal resource
"""
...
JOURNALS_COLLECTION = "dossiers"
JOURNALS_PATH = "/dav/journals/"


@rfc5545_bp.route("/dav/journals/", methods=["OPTIONS"])
@rfc5545_bp.route("/dav/journals/<dossier_id>.ics", methods=["OPTIONS"])
@dav_auth_required
def journals_options(dossier_id: str = None) -> Response:
    resp = Response("", status=200)
    resp.headers["Allow"] = "OPTIONS, GET, PUT, DELETE, PROPFIND, REPORT"
    resp.headers["DAV"] = "1, 2, 3, calendar-access"
    return resp
...
@rfc5545_bp.route("/dav/journals/", methods=["PROPFIND"])
@dav_auth_required
def journals_propfind_collection() -> Response:
    ...
    _add_journals_collection_response(multistatus, body)
...
@rfc5545_bp.route("/dav/journals/<dossier_id>.ics", methods=["PROPFIND"])
@dav_auth_required
def journals_propfind_resource(dossier_id: str) -> Response:
...
def _add_journals_collection_response(
    multistatus: ET.Element, body: ET.Element | None
) -> None:
    resp = add_response(multistatus, JOURNALS_PATH)
    prop = add_propstat(resp)
    ...
    if propfind_requests_prop(body, dav_tag("displayname")):
        ET.SubElement(prop, dav_tag("displayname")).text = (
            "Pallas Athena \u2014 Dossiers"
        )

    if propfind_requests_prop(body, cs_tag("getctag")):
        ET.SubElement(prop, cs_tag("getctag")).text = get_ctag(JOURNALS_COLLECTION)

    if propfind_requests_prop(body, dav_tag("sync-token")):
        ET.SubElement(prop, dav_tag("sync-token")).text = (
            f"data:,{get_sync_token(JOURNALS_COLLECTION)}"
        )
...
def _add_journal_resource_response(
    multistatus: ET.Element,
    dossier: dict,
    body: ET.Element | None,
) -> None:
    href = f"/dav/journals/{dossier['id']}.ics"
    resp = add_response(multistatus, href)
    prop = add_propstat(resp)
    ...
    if body is not None and propfind_requests_prop(
        ...
        ET.SubElement(prop, caldav_tag("calendar-data")).text = (
            dossier_to_vjournal(dossier)
        )


# ── REPORT (Journals) ─────────────────────────────────────────────────────

@rfc5545_bp.route("/dav/journals/", methods=["REPORT"])
@dav_auth_required
def journals_report_collection() -> Response:
    body_root = parse_report_body(request.get_data())
    if body_root is None:
        return Response("Bad Request", status=400)
    ...
    if local == "sync-collection":
        return _journals_sync_collection(body_root)
    elif local == "calendar-multiget":
        return _journals_multiget(body_root)
    elif local == "calendar-query":
        return _journals_calendar_query(body_root)

    return Response("Report type not supported", status=501)


def _journals_sync_collection(body_root: ET.Element) -> Response:
    token_el = body_root.find(dav_tag("sync-token"))
    client_token = ""
    if token_el is not None and token_el.text:
        client_token = token_el.text.replace("data:,", "")
    ...
    multistatus = make_multistatus()
    current_token = get_sync_token(JOURNALS_COLLECTION)

    if not client_token or client_token != current_token:
        dossiers = list_dossiers()
        for dossier in dossiers:
            resp = add_response(
                multistatus, f"/dav/journals/{dossier['id']}.ics"
            )
            prop = add_propstat(resp)
            ET.SubElement(prop, dav_tag("getetag")).text = (
                f'"{dossier.get("etag", "")}"'
        ...
        tombstones = get_tombstones(JOURNALS_COLLECTION)
        for ts in tombstones:
            add_status_response(
                multistatus, f"/dav/journals/{ts['id']}.ics", 404, "Not Found"
            )
...
def _journals_multiget(body_root: ET.Element) -> Response:
    multistatus = make_multistatus()

    for href_el in body_root.findall(dav_tag("href")):
        href = href_el.text or ""
        dossier_id = _extract_ics_id(href)
        if not dossier_id:
            add_status_response(multistatus, href, 404, "Not Found")
            continue
        dossier = get_dossier(dossier_id)
...
def _journals_calendar_query(body_root: ET.Element) -> Response:
    multistatus = make_multistatus()
    dossiers = list_dossiers()
...
@rfc5545_bp.route("/dav/journals/<dossier_id>.ics", methods=["GET"])
@dav_auth_required
def journals_get_resource(dossier_id: str) -> Response:
    dossier = get_dossier(dossier_id)
    if not dossier:
        return Response("Not Found", status=404)
    ...
    resp = Response(ical, status=200, content_type="text/calendar; charset=utf-8")
    resp.headers["ETag"] = f'"{dossier.get("etag", "")}"'
    return resp


# ── PUT (Journals) ────────────────────────────────────────────────────────

@rfc5545_bp.route("/dav/journals/<dossier_id>.ics", methods=["PUT"])
@dav_auth_required
def journals_put_resource(dossier_id: str) -> Response:
    if_match = request.headers.get("If-Match")
    if_none_match = request.headers.get("If-None-Match")
    ...
    if existing:
        updated, errors = update_dossier(dossier_id, data)
        if errors:
            return Response("\n".join(errors), status=422)
        bump_ctag(JOURNALS_COLLECTION)
        resp = Response("", status=204)
        resp.headers["ETag"] = f'"{updated.get("etag", "")}"'
    else:
        data["id"] = dossier_id
        created, errors = create_dossier(data)
        if errors:
            return Response("\n".join(errors), status=422)
        bump_ctag(JOURNALS_COLLECTION)
        resp = Response("", status=201)
...
@rfc5545_bp.route("/dav/journals/<dossier_id>.ics", methods=["DELETE"])
@dav_auth_required
def journals_delete_resource(dossier_id: str) -> Response:
    existing = get_dossier(dossier_id)
    if not existing:
        return Response("Not Found", status=404)
    ...
    record_tombstone(JOURNALS_COLLECTION, dossier_id)
    bump_ctag(JOURNALS_COLLECTION)
    return Response("", status=204)
