...
**Regarde ce qui est déjà fait avant de commencer.** Chaque ligne de
`list_documents` porte `analysee`, et avec elle la sous-nature, le niveau
de protection et `analyse_confirmee`. Saute ce qui est déjà qualifié : une
réanalyse écrit une entrée de plus au journal et brûle un appel que le
document suivant n'aura pas. Si l'avocat veut une reprise, il le dira.
...
**Un appel à `record_document_analysis` EST un enregistrement.** Il n'y a
pas d'aperçu : un seul appel par document, en écriture, avec une
`idempotency_key` qui lui est propre — c'est elle qui rend une reprise
inoffensive si le tour s'interrompt avant la fin du lot. Le nombre d'appels
de modèle par tour est PLAFONNÉ : chaque appel de trop coûte un document
que le lot n'atteindra pas.
