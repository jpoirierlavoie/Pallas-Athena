...
| `chat_model_empty` | **failure** | A Vertex response carrying NO content block. Emitted from the TRANSPORT (`chat/vertex.py`), before `chat_model_call` exists — which is why the 2026-08-31 incident left no line naming its own shape. `stop_reason`, `raw_stop_reason`, `blocks` (always 0), `input_tokens`, `output_tokens` |
| `chat_model_call` | success / failure | **One line per Vertex call** — the registre-completeness invariant made observable. `step`, `duration_ms`, `input_tokens`, `output_tokens`, `cache_creation_input_tokens`, `cache_read_input_tokens`, `web_search_requests`, `stop_reason`, `raw_stop_reason` (the PROVIDER's untranslated reason — two distinct Gemini reasons both translate to `end_turn`, and the log could not tell them apart after the fact; empty on the Anthropic path), `blocks` (content-block count) (a `pause_turn` is a success with its stop_reason) |
...
| `chat_tool_refused` | refused | Refusal before execution; `reason` ∈ `validation_failed`\|`gated_unattended`\|`unknown_tool`\|`write_disabled`. `validation_failed` was documented here but emitted NOWHERE until 2026-08-31: an argument refusal reached Cloud Logging as a plain `chat_tool_call`/failure, indistinguishable from a Firestore or Graph error, which is why eight consecutive refusals in one scheduled run raised nothing. It carries `arguments_refused` — the argument NAMES, which are schema, never content |
...
| `chat_web_search` | success | **One line per issued web search.** `step`, `tool`, `query_chars`, `query_sha8` — and deliberately NOT the query itself: it is model-composed and can carry a client name straight out of a privileged thread, which the redaction filter does not scrub. The text stays on the turn document for a forensic read. This is the only detection surface that exists for the channel: a `server_tool_use` block is already a RESULT, so the query left the tenant before any code in this repo saw it |
| `chat_mail_unavailable` | refused | Emitted ONCE per process when `CHAT_MAIL_ENABLED` is true but the mailbox is not configured. « Not configured » is the normal, silent state on the default and portail services; « enabled and unconfigured » means the mail tools vanished from the model's array for a reason nobody can see — the shape of the origin-secret defect that stayed silent for months. Carries `reason` only |
...
| `chat_authorization` | success / refused | The §4.6.3 seam; `tool`, `decision` ∈ `approuve`\|`refuse` — the refusal tool_result is the FIXED French string, never quoted content. Depuis le 2026-08-31, `decision="lot_execute"` marque CHAQUE exécution d'un lot approuvé (`step`, `tool_use_ids`) : deux lignes de même `turn_id` ET même `step` veulent dire qu'un lot a tourné DEUX FOIS — le filet de détection du seul vecteur que le correctif ne ferme pas (deux redélivrances concurrentes réclamant le même jeton). L'écriture elle-même est indiscernable d'une écriture légitime |
...
| `chat_report_emailed` | success / refused / **failure** | `deliver_email` path; `refused` + `reason="graph_not_configured"`, `failure` + `reason="graph_error"` (the in-app copy remains — the accusé posture). Since 2026-08-31 the FAILURE NOTICE shares this event and the same at-most-once marker: `failure` + `reason` ∈ `aucun_rapport`\|`registre_illisible`\|`echec`\|`avis_non_livre` — a scheduled run that produced nothing says so in the inbox rather than going silent |
