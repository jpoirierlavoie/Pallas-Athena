...
INSTRUCTIONS = (
    ...
    "manager. 23 tools read; 10 write. Every write is CREATE-ONLY except "
    "`complete_task`, which closes a task (terminée / annulée / en_cours) "
    "and is the ONLY status change available here. The create-only ones: "
    ...
    "the `athena:write` scope. Nothing can be DELETED, nothing else can be "
    "edited, and invoices, trust accounting, documents and contacts are "
    "never writable. One indirect effect to know: completing a task that a "
